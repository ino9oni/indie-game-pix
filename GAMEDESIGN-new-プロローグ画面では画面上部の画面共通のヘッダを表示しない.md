...
- 表示
  ...
  - コレクションボード（右カラム）
    - 木目調の四角いボードパネル（推奨比率 4:3、最小幅 420px）を描画し、内部に5列×6行のグリッド（計30スロット）を配置する。
    - 各スロットは丸みを帯びたソケット（直径72px想定）で表現し、未解放時は透過度60%、解放時は金色のリングと発光を付ける。
    - 難易度ごとにスロットへ固定割り当てを行い（practice列→最上段左から順に埋まる）、カーソル／タップ時に `glyphMeta.meaningText` をツールチップで表示する。
    - 今回新たに埋まったスロットは 0.6s のポップアップアニメーション（スケール1.2→1.0、アルファ1.2→1.0）を再生し、既存のスロットは静的状態で表示する。
    - サイズ別ボード構成
      - 5×5 用ソケット群（PRACTICE〜MIDDLE）は既存の5列×6行グリッドを継承し、右カラム上段に配置する。ボードタイトルを「Elfpix Symbol 5×5」にし、10px 程度の装飾見出しを追加する。
      - 10×10 用ソケット群（HARD / ULTRA）は右カラム下段に独立したボードを用意し、5列×2行の計10スロットで構成する。タイトルは「Elfpix Symbol 10×10」とし、5×5 ボードと同一幅で配置する。
