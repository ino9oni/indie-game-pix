          ...
          {blocks.map((rows, bi) => (
            ...
            >
              ...
              {bi === 0 && (
                ...
                >
                  <button className="kana" onClick={backspace}>
                    一文字削除
                  </button>
                  <button
                    className="kana"
                    onClick={() => setCaret((c) => Math.max(0, c - 1))}
                  >
                    ←
                  </button>
                  <button
                    className="kana"
                    onClick={() =>
                      setCaret((c) => Math.min(name.length, c + 1))
                    }
                  >
                    →
                  </button>
                  <button
                    className="kana"
                    onClick={() => onConfirm(name || "ナナシ")}
                  >
                    決定
                  </button>
                  {onCancel && (
                    <button className="kana" onClick={onCancel}>
                      戻る
                    </button>
                  )}
                </div>
