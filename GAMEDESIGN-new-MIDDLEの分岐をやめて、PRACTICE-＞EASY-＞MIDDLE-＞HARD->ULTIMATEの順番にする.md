...
- 目的：分岐のない一本の進行ルートに再設計し、進行順を明確にする。
- 対象：ルートマップのノード配置／接続順序／エッジレイアウト。
- 変更内容：
  - 分岐を廃止し、`PRACTICE → EASY → MIDDLE → HARD → ULTIMATE` の一本道にする。
  - 既存の開始／終端（START/ENDING）前後の流れは維持しつつ、難易度の並びのみ上記に統一する。
  - ノードが5つ連続するため、エッジ長を短縮し、画面内で折れ曲がらないことを優先する。
  - ルートの進行方向は既存の右上方向を踏襲し、ストーリーは前後関係が破綻しないよう連続させる。
