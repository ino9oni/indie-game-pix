...
- 表示
    - 主人公の表示
        - 画像ファイル：主人公の普通画像の表示
        - ラベル表示：${HERO_NAME}
        - レイアウト：左端（会話ウィンドウの左上位置）
    - 敵の表示
        - 画像ファイル：敵キャラクターの普通画像の表示
        - ラベル表示：${ENEMY_NAME}
        - レイアウト：右端（会話ウィンドウの右上位置）
    - 会話のウィンドウ
        - 会話の表示
            - 主人公
                - ウィンドウの左上：セリフの話者の名前
            ...
            - 敵
                - ウィンドウの右上：セリフの話者の名前
                - ウィンドウの内部：セリフ本文
    - キャラクター名と感情にあわせた画像を読み込む・表示する。
        - 主人公
            - 怒:hero/hero_angry.png
            - 普:hero/hero_normal.png
        - 敵
            - 怒:enemies/${LEVEL}/${ENEMY_NAME}_angry.png
            - 普:enemies/${LEVEL}/${ENEMY_NAME}_normal.png
- 操作
...
- 内部状態
    - 会話スクリプトと画像を読み込む。
        - practice の会話
            ...
            - No.5
                - 話者：敵キャラクター
                - 台詞："${HERO_NAME}と言うのね、いい名前ね。覚えておくわ。私は${ENEMY_NAME}よ、覚えてくれると嬉しいわ。"
            - No.6
                - 話者：敵キャラクター
                - 台詞："では準備はいい？いくわよ。"
                - 感情：怒
