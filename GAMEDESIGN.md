...
- 概要
  ...
  - パズル生成
    - 各バトル開始時に、難易度ごとのテンプレート群からランダムにパズルを生成する。テンプレートは既存のモチーフ（紋章・植物・シンボルなど）を保持したまま、回転・反転・平行移動を組み合わせてバリエーションを作る。
    - 生成時は左右／上下の完全対称となる組み合わせを避け、同一セッション内で同じレイアウトが重複しないようにする（ハッシュ管理で重複を排除）。
    - 生成した盤面は `computeClues` 相当のヒントを算出し、バックトラッキング型ソルバで唯一解かを検証する。複数解が見つかった場合は再生成する。
    - HERO と ENEMY は同じ生成済みパズル集合を共有しつつ、提示順はそれぞれ別途シャッフルする。これにより出題内容は同一でも、解く順番が異なる。
    - 生成ロジックは乱数シードを受け取り（指定が無い場合は毎回ランダムシード）、デバッグ用途で再現性が必要な場合に備える。
