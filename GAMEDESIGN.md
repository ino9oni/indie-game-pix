    ...
    - practice
        ...
        - 対戦相手
            ...
            - 属性
                ...
                - 画像（通常）：assets/img/enemies/00011-3639403992.png
                - 画像（笑顔）：(test画像)
                - 画像（怒り）：(test画像)
    - easy
        - ノードとの対応
...
## Cross Platform 
- webアプリケーションのビルドを行う
- androidアプリケーションのビルドを行う
    - webアプリケーションをandroidでも実行可能なように変換する
    - 変換後のソースは、リポジトリの中に「mobile/android」フォルダを作成し、その中に生成する
## Test Process
    - no definition(under construction)
## CI/CD Process
- コミットの作法
    - GAMEDESIGN.mdファイル（このファイル）に変更がある場合ステージします
    - ステージされたファイルを
- バージョニング
    - Commitする際に、バージョンについては提案をしてください。
    - マイナーバージョンアップ
        - 定義
            - バグ修正、UIの表示程度の変更の
            - UIの表示位置の変更
    ...
    - メジャーバージョンアップ
        - 定義
            ...
            - データアクセスの変更
- 通知
    - 変更点をコミットする際に差分を教えてください。
    - 差分をユーザが確認し・承認を確認します。
    - 承認後、コミット・Pushしてください。
    - ユーザが承認しなかった場合は、承認されなかった理由の入力を求めます。
    - 入力に基づき修正を行います。

- コミットメッセージの定義
    - 形式
        - "[modified] ${バージョン} ${変更内容のサマリ}"とします。
        - 変更内容のサマリはCodexで修正したTODの内容を記載します。

- タグ
    - メジャーバージョンが上がった場合に、タグ漬けをします
    - タグの形式
        - コミットメッセージと同様
