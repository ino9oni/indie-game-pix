...
- 対応デバイス：Xbox系（A/B/X/Y）および PlayStation 系（×/〇/□/△）の標準ゲームパッド
- ボタン割り当て
  - `A / ×`：決定（Confirm）。メニュー選択、テキスト送り、文字入力など肯定的な操作全般に利用する
  - `B / 〇`：キャンセル（Cancel）。スキップ、一文字削除などの取り消し操作に利用する
  - `START`：ポーズ／再開（既存のピクロスモード仕様を踏襲）
  - `SELECT`：盤面リセットなどの補助操作（既存仕様を踏襲）
- 方向入力
  - 十字キー（D-Pad）または左スティックでフォーカス対象を移動する
    - D-Padは Gamepad API の `buttons[12-15]` をそのまま利用し、押下1回で1つ分移動する
    - 左スティックは軸値の絶対値が0.5以上になった場合に1回の入力として扱い、離された状態へ戻るまで再入力を無効化する
  - 縦並びのメニューは上下、横並びのボタンは左右で遷移させる
- フォーカス表示
  - ゲームパッド入力が最後に検知された状態（`inputMode === "gamepad"`）では、フォーカス中の要素に `focused` 表示を適用してハイライトする
  - フォーカスは各モードの主要操作ボタン（Next / Skip / New Game / Continue / かなボタン 等）で一貫して表示する
- マウス・タッチ操作を検知した場合はフォーカス表示を解除し、以降はマウス／タッチ操作を優先する
  - 各モードに入った際、初期フォーカスが必ず設定されるようにする（例：プロローグは Next、オープニングは New Game、名前決定は左上のかな）
...
- 画面表示
  ...
  - プロローグ表示画面
    ...
    - 入力
      ...
      - ゲームパッド：`A / ×` で Next ボタンを押下し、現在の行を即時全表示 → 再度の押下で次の行へ進める
      - ゲームパッド：`B / 〇` で Skip ボタンを押下し、以降の行を表示せずにオープニングへ遷移する
      - プロローグ突入時は Next を初期フォーカスとし、左右入力で Skip へ移動できるようにする
    - 自動
      - テキストがすべて表示されたあと、2秒待つ
      - **オープニング** に移行
    - スキップ操作
      - ウィンドウ右下に「Next」「Skip」ボタンを横並びで配置する
        - Next：表示中の行を即座に全表示し、次の行へ進める。最終行で押下した場合は**オープニング**へ遷移する
        - Skip：残りの行を表示せず即座に**オープニング**へ遷移する
      - ゲームパッド入力時は左右入力で Next / Skip のフォーカスを切り替え、決定ボタンで押下する
...
- 概要
  ...
  - 操作
    - ゲームパッド入力時、十字キーまたは左スティックの上下で `New Game` / `Continue` のフォーカスを切り替える
    - 決定ボタン（`A / ×`）でフォーカス中のメニューを実行する
    - キャンセルボタン（`B / 〇`）はオープニングでは利用しない（入力しても無視）
    - フォーカス中のボタンは `focused` スタイルで強調表示し、スクリーンリーダー向けに選択中ラベルを付与する
    - オープニング突入時は `New Game` を初期フォーカスとし、上下入力は循環する（`Continue` から下を押すと `New Game` に戻る）
...
- 操作
  ...
  - 取り消しボタン
    ...
    - １文字入力を戻す
  - ゲームパッド操作
    - 方向入力（十字キー／左スティック）でかなボタンおよび操作パネル上のボタン間を移動する
      - グリッド状にフォーカスが移動し、縦横どちらの入力でも隣接するボタンへ遷移できるようにする
      - 操作パネル内とかなブロック間の移動も連続して行えるようにする
      - 画面遷移直後は左上（「ア」）を初期フォーカスとし、上下・左右入力はそれぞれの行／列で循環する
    - 決定ボタン（`A / ×`）でフォーカス中のボタンを押下する
      - かなボタン：該当文字を入力し、カーソルを1文字分右へ移動する
      - [←][→]：入力カーソルを左右へ移動する
      - [決定]：入力内容を確定し、モードを終了する
      - [戻る]：名前入力をキャンセルし、モード開始前の画面へ戻る
    - キャンセルボタン（`B / 〇`）で `一文字削除` を実行する（入力が空の場合は無操作）
    - ゲームパッド入力時は現在フォーカス中のボタンを `focused` スタイルで強調表示する
...
- 操作
  - ノードをクリック
    ...
    - 演出
      ...
      - 移動中は他ノードをクリックできない
      - 到着直後、ノードの上に「Enemy Encount！」と記載した小さな吹き出しを約0.8秒表示し、その後に会話モードへ遷移する
        - エンディングノード（elf-bad-ending, elf-true-ending など type: "end" のノード）に到達した場合はこの吹き出しを表示しない
      - エンカウント演出
      - 吹き出しレイアウト
        - 「Enemy Encount！」ラベルは文字左右に30pxの余白を確保し、吹き出し幅160px以上を維持して読みやすさを確保する
        - 吹き出し表示と同時にマップモードの舞台（ルート図と背景）全体へ0.12秒で倍率1.0→1.15の拡大を掛け、同じ速度で1.0へ戻す動きを2回繰り返す（ease-in-out）
        - 拡大ピークに合わせて `#ff4d4d` ベースの不透明度30%の半透明レイヤーを全画面に乗せ、0.1秒で点滅させて遭遇感を強調する
        - 演出の総時間は約0.7秒とし、アニメーション終了時にレイヤーを即座に非表示へ戻す
        - 演出処理が完了したタイミングで会話モード遷移処理を実行する（0.8秒の吹き出し表示時間内に収める）
...
- 会話モードのキャラクターの表示サイズを、縦横1.5倍にする
  - 標準フレーム幅を570px（高さ731px）とし、画面幅に合わせて縮小しつつ比率を維持する
