  ...
  - カタカナを横書きで並べてある
    ...
    - レイアウト
      ...
      - 文字
        ...
        - レイアウトは５文字ずつ揃える
  - 背景画像
    - 名前入力モードでは assets/img/character/hero/hero_fullbody.png を背景に固定表示する
      - 透明度：60%
...
- 概要
  ...
  - レイアウト
    ...
    - 盤面上部に共通タイマー、ラウンド表示（Puzzle 1/3 など）、勝敗カウンタ（プレイヤー/敵のクリア数）を表示する。
    - 各陣営の進捗を示すバーを設置し、正解マスの埋まり具合を即時に視覚化する。
    - スペル発動時は画面中央上部にアンギー画像＋スペル名をオーバーレイ表示し、ログにも履歴を残す。
    - プレイヤー盤面の左上に主人公の通常ポートレート（高さ100px）をカード風に重ねて表示し、縦横比を保持する。
    - 敵盤面の左上に敵キャラクターの通常ポートレート（高さ100px）を同様に表示し、会話モードと同じ立ち絵を用いる。
...
- 操作
  ...
  - 自動クリア判定
    ...
    - 自動クリア時も手動Submitと同じスコア加算・演出・次パズルへの遷移フローを適用する
  - リアルタイム正解ボーナス
