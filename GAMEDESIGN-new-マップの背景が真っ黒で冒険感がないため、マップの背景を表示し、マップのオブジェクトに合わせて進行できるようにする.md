...
### タスク仕様（マップ背景の表示と進行オブジェクトの整合）

- 目的：黒背景で没入感が落ちる問題を解消し、マップ背景とルートの進行が視覚的に一致するようにする。
- 対象：ルートモード（マップ画面）の背景表示、ノード/エッジ配置。
- 仕様：
  - マップモードでは `assets/img/background/map.png` を背景に固定表示する。
  - マップ背景のランドマーク（道・分岐・拠点）に沿ってノード/エッジの座標を調整する。
  - 既存のズーム/スクロール/エンカウント演出は維持し、背景が真っ黒になる状態を解消する。
  - ノード配置は以下に合わせる：
    - Start：画面左下（キャンプファイヤー付近）
    - 翠緑の門：画面左下の門
    - 木漏れ日の小径：画面右下の森
    - 刻印の広間：画面左上の門
    - 精霊の花園：画面中央の花園
    - 森羅の社：画面右上の石柱
    - 黎明の庭：画面右上の石造建物
  - 「森羅の社 → 黎明の庭」のエッジは未訪問時でも道色（濃い茶色）で表示する。
