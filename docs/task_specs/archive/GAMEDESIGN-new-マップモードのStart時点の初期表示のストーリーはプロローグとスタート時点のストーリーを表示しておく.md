...
- ゲーム開始後にマップへ遷移した時点で、ストーリーブロックに「プロローグ」と「スタート直後」のストーリーを表示する。
- マップ初期表示ではストーリーブロックが空にならないようにし、初期ストーリーを先に流した状態から開始する。
- PRACTICE ノード以降のストーリーは、そのノードのイベント／勝利後に追加する（初期表示では追加しない）。
