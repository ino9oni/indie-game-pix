...
## タスク仕様（画像・音楽の事前ダウンロードとキャッシュ）

- 画像・音楽などの静的アセットを初回起動時に事前ダウンロードし、再生/表示時の遅延をなくす。
- ダウンロード対象は `public/assets/` 配下の画像・音楽・SE を中心に、ゲーム中に参照する全アセットとする。
- 事前ダウンロードはゲーム開始前（起動直後〜タイトル画面中）にバックグラウンドで行い、ゲーム操作を極力ブロックしない。
- 取得したファイルはブラウザのキャッシュ（Cache Storage など）に保存し、以後はキャッシュから取得する。
- キャッシュが存在する場合は再ダウンロードをスキップし、起動時間を悪化させない。
