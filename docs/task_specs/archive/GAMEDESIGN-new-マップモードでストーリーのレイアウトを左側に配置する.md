...
### タスク仕様（マップモードでストーリーのレイアウトを左側に配置する）

- 目的：ストーリーの流れをマップと同一方向に感じられるようにし、進行の視認性を高める。
- 対象：ルートモード（マップ画面）の UI レイアウト（マップ/ストーリーパネルの配置）。
- レイアウト変更：
  - デスクトップ幅では 2 カラムを維持し、**左：ストーリー 25%／右：マップ 75%** に配置する。
  - 配置比率・最小幅・高さ揃え・内部スクロールの仕様は既存のまま継続する。
  - 狭幅（モバイル）時の縦スタックや内部スクロール挙動も維持する。
