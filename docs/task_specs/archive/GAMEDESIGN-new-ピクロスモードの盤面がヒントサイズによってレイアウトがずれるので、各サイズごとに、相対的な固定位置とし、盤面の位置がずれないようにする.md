...
- Objective: ヒント文字幅の違いで盤面位置がズレないよう、盤面サイズ別に固定アンカーを定義する。
- Non-goals: ヒント内容やパズル生成ロジックの変更、UIテーマの刷新。
- Affected files: src/styles.css, src/App.jsx（もしくは盤面コンポーネント一式）。
...
- 静的アセットはリポジトリ内の `public/assets/` に配置し、アプリケーションからは `/assets/…` パスで参照します。

## Gamed design and specification

### タスク仕様（ピクロス記号化・コレクション強化）

- ピクロスモードの各ノードで出題するお題数を `practice:2 / easy:4 / middle:6 / hard:8 / ultra:10` に固定し、テンプレートの `collectionIndex` 昇順で出題する。
- テンプレートは意味付けされた 5×5 グリッドを正規形のまま使用し、勝利演出やコレクション表示で同じシルエットを共有する（回転・反転・合成は行わない）。
- コレクション進行データには解放フラグに加えて `unlockedAt`（ISO8601）と正規化グリッドを保存し、勝利画面で曜日に依存しない形式（`YYYY/MM/DD`）で表示する。
- ピクロスクリア画面では `"<HERO_NAME> WIN"` に続けて「新たな記号：...」形式で今回獲得したシンボル名を1行表示し、複数の場合は読点区切りで列挙する。
- Elfpix Symbol ボードは 5×6=30 スロットを保持し、各スロットに `glyphLabel`・難易度タグ（例：`EASY-03`）・解放日付を表示、未解放は `??`・解放済みは収集済みシルエットを点灯させる。
...
### タスク仕様（勝利画面のデザインや文言の改良）

- 勝利画面右カラムに表示するコレクションボードのタイトルテキストを「Elfpix Symbol」に変更し、画面内で同じ表記に統一する。
- コレクションボードのグリッドは 5 列 × 6 行を基準構成とし、30 個のシンボル表示時も 6 行目までを使用して配置する。
- コレクションボードの表示幅はビューポート幅の 70% 前後を目安とし、HERO セクションと並列配置しても各スロットが欠けないサイズを確保する。
- 表記ゆれ防止のため、ボード見出し・関連ラベル・ARIA ラベルも新名称「Elfpix Symbol」に合わせて更新する。
- 既存の Symbol Codex 表記を使用している他モード／画面がないか確認し、影響がある場合は「Elfpix Symbol」に統一する。

### タスク仕様（デバッグモードの改善）

- 既存のトップアクション領域にあるデバッグモード切替ボタンは撤去し、通常 UI にデバッグ専用要素を残さない。
- `Ctrl` キーを単押しすると、ポインタ位置を基準に簡易コンテキストメニュー型の「デバッグメニュー」を表示する。
  - メニュー表示中に再度 `Ctrl` を押下、もしくはメニュー外をクリックする／Escape を押すとメニューを閉じる。
  - マウス操作を基本としつつ、`Tab` と `Shift+Tab` でアイテム間を巡回、矢印キーでリスト選択、`Enter` / `Space` で決定できるようにする。
- メニューはどの画面（screen）にいても呼び出せ、以下のアクションを提供する。
  1. **マップへ移動** — ルートノード一覧（start/elf-practice/…）から目的地を選択すると、該当ノードまでをクリア済み扱いにした上で `route` 画面へ遷移し、選択ノードを現在地にする。
  2. **会話へ移動** — エネミーノードを選択すると、その会話開始時点に必要な状態（`pendingNode` 設定など）を再構築して `conversation` 画面を表示する。
  3. **ピクロスへ移動** — エネミーノード選択で `picross` モードへ直行し、該当ノード向けのパズル生成ロジックを通常通り呼び出す。
  4. **勝利画面を確認** — ノード選択後、直近勝利結果を仮生成して `picross-clear` 画面を表示し、コレクション解放処理は実行しない。
  5. **エンディングへ移動** — エンディング種別（true/bad）を選択し、必要ステートを整えて `ending` 画面へ遷移する。
  6. **ENEMYコンボ +1** — ENEMY のコンボ数を 1 だけ加算し、必要なら次のコンボステージを解放する。
  7. **ENEMYコンボ MAX** — ENEMY のコンボ数を最大（最上位ステージ到達）に設定する。
  8. **ENEMY盤面 90%** — ENEMY の盤面を 9 割クリア状態に更新する。
  9. **ENEMY次のお題へ** — ENEMY の現在の回答済みお題に +1 して次の問題へ進める。
  10. **HEROコンボ +1** — HERO のコンボ数を 1 だけ加算し、必要なら次のコンボステージを解放する。
  11. **HEROコンボ MAX** — HERO のコンボ数を最大（最上位ステージ到達）に設定する。
  12. **HERO盤面 90%** — HERO の盤面を 9 割クリア状態に更新する。
  13. **HERO次のお題へ** — HERO の現在の回答済みお題に +1 して次の問題へ進める。
- メニュー内で「即時クリア」トグルを維持し、オンの場合は従来通り回答送信時に盤面を即クリア扱いにする。トグル状態はローカルストレージに保存し、従来の `debugMode` フラグを兼ねる。
- メニュー表示時は UI フレームを暗くする等の演出は行わず、操作後は自動的に閉じる。フォーカス管理と ARIA 属性を付し、アクセシビリティ上の破綻が無いことを確認する。

### タスク仕様（エピローグのクレジット画面にSpecial Thanks追加）

- エピローグのクレジット画面に「Special Thanks」セクションを追加する。
- セクションタイトルは「Special Thanks」と表示する。
- ロール表記は「TestPlayer」、名前は「sideofall」とする。
- 既存クレジットのレイアウト／フォント／余白を踏襲し、他項目と同じスタイルで表示する。

### タスク仕様（マップモードのアイコン拡大とエッジ強調）

- マップモードのノードアイコン（拠点/敵/分岐）を現状の2倍サイズに拡大する（直径/幅/高さ）。
- アイコン拡大に伴い、当たり判定／クリック領域も同スケールで合わせる。
- ノード間のエッジ（ルート線）を道らしく太くする（線幅を2倍程度）。
- アイコン拡大とエッジ強調により見切れや重なりが出ないよう、必要に応じて余白や配置を調整する。
- マップモード以外のUIには影響させない。

### タスク仕様（ENEMY側スペル発動率の改善）

- ENEMYのコンボカウントが溜まったら、ENEMYがジャマ―（スペル）を発動する。
- ENEMY側も解答やMAYBEが正しく合っていればコンボカウントを計上する。
- ジャマ―の攻撃種類帯はレベル1〜3程度に抑える。

### タスク仕様（Reset/Submit/Quitボタン非表示とPause/Quit統合）

- ピクロス対戦中のトップアクションにある Reset / Submit / Quit ボタンを常時非表示にする。
- Pause ボタンの表示を「Pause/Quit」に変更し、押下時は一時停止オーバーレイを表示する。
- 一時停止オーバーレイ内に「Quit」ボタンを表示し、そこからタイトルへ戻れるようにする。
- キーボードの Esc キーでも Pause/Quit と同等の一時停止オーバーレイを表示する。

### タスク仕様（ピクロスお題数と盤面調整）

- 各難易度ごとの対戦お題数を `PRACTICE=2 / EASY=4 / MIDDLE=6 / HARD=2 / ULTRA=4` に固定する。
- HARD/ULTRA 難易度のピクロス盤面は 10×10 を標準とし、HERO/ENEMY 双方で同一サイズを採用する。PRACTICE/EASY/MIDDLE は従来どおり 5×5 を維持する。
- 生成ロジックは不足分が発生した場合でも fallback で補完し、常に上記件数のヒーロー/ENEMY お題を用意すること。
- 難易度ごとにテンプレートプールを明確に分離し、同じ実行中に PRACTICE/EASY など異なる難易度で同一テンプレートが連続使用されないようにする。
- `PUZZLES_PER_BATTLE_BY_NODE` などの定数表から参照する形を維持し、将来的なノード追加でも同様の設定で拡張できるようにする。

### タスク仕様（ピクロス盤面レイアウト固定）

- 対象：ピクロスモードの HERO / ENEMY 盤面とヒント領域。
- 盤面サイズ（5×5 / 10×10）ごとに、盤面＋ヒントのグリッド枠を固定寸法で定義し、ヒント文字幅の違いで盤面位置がズレないようにする。
- お題変更でヒントの行数が増減（例：2行→3行）しても、盤面の左上アンカー位置は固定し、盤面位置が上下に動かないようにする。
- ヒント行数の最大値（盤面サイズごとの最大ヒント数）を算出し、常に「最大行数を前提にしたヒント領域サイズ」でレイアウトする。
- HERO と ENEMY の盤面位置は同一のアンカー基準で揃え、左右の列で上下ズレが発生しないようにする。
- 実装指針：
  - 盤面コンテナを `grid-template-columns: var(--hint-col-size) var(--board-size)`、`grid-template-rows: var(--hint-row-size) var(--board-size)` で構成し、盤面の左上アンカーを固定する。
  - 5×5 と 10×10 で `--hint-col-size` / `--hint-row-size` を個別指定し、ヒントの桁数に影響されない固定幅にする。
  - ヒント数値の幅ぶれを抑えるため `font-variant-numeric: tabular-nums` を適用する。
  - デスクトップ／モバイルで縮小する場合もアンカーは維持し、盤面全体をスケールしても中心位置が変わらないようにする。

### タスク仕様（中級以上ピクロスのお題難化）

- MIDDLE 以上のテンプレートでは、行・列ヒントの最大値が盤面サイズの 20% を超えるパターンを禁止し、大きなヒント数は各方向で 2 本以内に抑える。
- HARD/ULTRA ではテンプレート生成時に左右対称・上下対称パターンを避け、最終グリッドの反転チェック（左右・上下・180°）で一致する場合は再生成する。
- MIDDLE/HARD テンプレートは、盤面の 25〜45% を塗りつぶしセルとし、連続ブロックが 2〜4 に収まるようヒント配列を再構成する。
- HARD/ULTRA テンプレートは 10×10 を前提とし、塗りつぶし密度 35〜55%、5 本以上のヒントが並ぶ行列を 2 本以内に制限する。
- HARD/ULTRA（10×10）では行・列ヒントの出現割合を `10:10% / 7〜9:10% / 4〜6:40% / 2〜3:30% / 1:10%` を目安に調整し、過剰な長尺ヒントが連続しないよう生成後に正規化する。
- 既存テンプレート定義に難易度タグ（`glyphMeta.difficultyTier` など）を追加し、生成処理側で MIDDLE/HARD/ULTRA 使用分に対してヒント正規化を適用する。
- テンプレートの乱択前に、行列ヒント配列から難易度スコア（平均ヒント長 × 分散）を計算し、MIDDLE 以上では閾値未満のテンプレートを除外する。
...
- 基本方針：
  ...
  - 誤答検知とロールバック：
    - 推定手で置いたセルが矛盾を招いたと判断できる場合は、適用直前のスナップショットに巻き戻して別候補を再検討する。
    - 連続で進展が無い/矛盾が出たときは「推定→矛盾→ロールバック」のループに陥らないよう、同一セル/同一推定の再試行を禁止し、別セルへ切り替える。
    - ロールバック回数に上限を設け、上限到達時は情報量スコアが最も高い未確定セルに推定を切り替えて探索を続行する。
    - ラインがヒント充足で確定したら残マスを即×埋めし、盤面評価を軽くして永続ループを防ぐ。
- デフォルト値（各難易度ベース、今後個別敵IDで上書き可能）：
  - Practice: intervalRange 1.2〜1.6s, successRate 0.88, errorRate 0.12, spellChance 0.35
  - Easy: intervalRange 1.0〜1.4s, successRate 0.9, errorRate 0.1, spellChance 0.35
  - Middle: intervalRange 4〜9s, successRate 0.82, errorRate 0.16, spellChance 0.4
  - Hard: intervalRange 3.5±2s（1.5〜5.5s）, successRate 0.8, errorRate 0.25, spellChance 0.45
  - Ultra: intervalRange 1.7±0.5s（1.2〜2.2s）, successRate 0.9, errorRate 0.08, spellChance 0.6
- 拡張性：
...
- 目的：スペル発動演出をより「相手ボードへの攻撃」として体感させ、台詞→演出→ボード被弾→復帰の流れを明確にする。
- 対象：HERO／ENEMY のスペル発動（全難易度共通）。
- 変更内容：
  1. シネマティック導入  
     - 発動時にタイマー・入力を一時停止し、全画面シェーダー／グラデーションで舞台を暗転。  
     - HERO は画面左から、ENEMY は右からスライドインし、全身が 80〜100% の高さで映るレイヤーを表示する。  
     - スペル名＋短い台詞をバブル表示し、少なくとも 800〜1200ms は読める時間を確保する。
  2. ボード攻撃シーン  
     - 台詞後は対戦相手のピクロスボード以外を暗転させ、相手ボードに 3D 風の攻撃エフェクト（ビーム／斬撃／衝撃波などのいずれか）を重ねる。  
     - エフェクト発生中は攻撃対象セル周辺にハイライト・震え・フレアを付与し、「どこに当たったか」が視覚で分かるようにする。
  3. 結果提示と復帰  
     - 攻撃エフェクト終端で 400〜600ms 静止し、被弾後のボード状態を見せる。  
     - その後シェーダーをフェードアウトし、タイマー・入力を再開する。  
     - ミュート設定や演出スキップ（従来のスキップ手段）がオンの場合は、導入・攻撃を即時適用しログ／アイコンのみで結果を示すが、効果自体は同じタイミングで適用する。

### タスク仕様（マップモードの画面右にここまでのストーリーを表示する）

- 目的：マップ画面で進行中のストーリーを常時参照できるようにし、分岐や進行状況の把握を補助する。
- 対象：ルートモード（マップ画面）の UI レイアウト、ストーリー表示パネル、ストーリーブロック（章カード）。
- レイアウト：
  - デスクトップ幅では 2 カラムで表示する（左：マップ 75%、右：ストーリー 25%）。
    - ストーリー欄は最小幅を確保し、極端に狭くなった場合はレイアウトが破綻しないこと。
    - ストーリー枠の高さは左のマップ枠と揃え、内容が溢れた場合は枠内スクロールで閲覧できること。
  - 狭幅（モバイル）では縦にスタックし、画面スクロールではなく「ストーリー欄の内部スクロール」で閲覧できるようにする。
  - ルートモード中は画面全体をビューポート内に収め、ウィンドウスクロールを発生させずに UI が完結する。
- ストーリーパネル（右側の本風 UI）：
  - タイトル「ここまでのストーリー」と表示/非表示トグルを持つ（非表示時はヘッダーのみを残す）。
  - 背景は「書籍・古文書」風（ページ色、紙のにじみ、綴じ目の陰影など）とし、森と elfpix の世界観に合う配色にする。
  - フォントは明朝/セリフ系（古文書感）を基本とし、本文は全体の 0.75 倍程度のサイズ感で読みやすくする。
- ストーリーの積み上げ（ログ）：
  - ストーリーは章（ストーリーブロック）単位で表示し、最新の章が末尾に追加されていく。
  - 既に表示した章は再表示・重複を行わず、追加分のみを新規に追記する。
  - ブランチの未踏ルート（主人公が進んでいない方向）の章は表示しない。
  - ストーリーの表示条件は「ノードを選択する前の予告」ではなく「クリア後に解放された章」を基本とする。
  - ログはローカルストレージに保存し、再起動時に復元できる。
- ストーリーブロック（章カード）：
  - 各章は「サブタイトル（章タイトル）」＋複数段落の本文で構成する。
  - 段落表示は 1 文字ずつ左→右へフェード（軽いスライド）する演出を許可し、章追加時はカード全体もフェードインする。
  - キャラクター画像を表示する場合、ストーリーブロックの高さを押し広げないよう透過/ウォーターマーク（薄い不透明度、背面合成）として重ねる。
- スクロール：
  - ストーリーブロックが増えて表示領域を超えた場合、ストーリー欄に常時表示のスクロールバーを出し、内部スクロールで閲覧する。
  - 新しい章が追加されたタイミングで自動的に最下部までスクロールして最新章を見せる。

### タスク仕様（Elfpix世界観に沿ったUIカラーパレット）

- テーマコンセプトを「深い森と星明かり」に定め、下記 4 レイヤーのカラーパレットを導入する。
  1. **Background / Surface**: `#050912` をベースに、セクション背景は `linear-gradient(140deg, #050912, #0e1931)`、カード背景は `rgba(10, 20, 45, 0.85)` を用いる。
  2. **Primary Accent**: 精霊の光を想起させるティール〜シアン（例：`#19e5c2`, `#3fe7ff`）を CTA ボタン、重要リンク、メーターのアクティブ色に使用。
  3. **Secondary Accent**: 深藍〜紫（例：`#5a56e0`, `#8c6df2`）をサブボタンやヘッダー装飾に使用。
  4. **Warn/Danger**: 炎／毒に対応する暖色（`#f27f60`, `#e75378`）を敵側アラートや危険操作に使用。
- CSS 変数（例：`--surface`, `--surface-elevated`, `--accent-primary`, `--accent-secondary`, `--accent-warn`）を `:root` に定義し、既存の主要コンポーネント（ヘッダー、ボタン、ダイアログ、ボードフレーム、HUD）で変数参照へ置き換える。
- ボタン／入力コンポーネントのフォーカスリングは `#9ef6ff` とし、キーボード操作時に 2px のアウトラインが視認できるようにする。文字色は `#e6f4ff` を基本とし、背景とのコントラスト比 4.5:1 を確保する。
- 背景イメージやグラデーションはフェード演出時の輝度差が極端にならないよう、アルファ値を `0.65` 前後に統一し、画面遷移でも色の統一感を維持する。
...
- 会話ウィンドウを表示
  - 画面
    - ウィンドウ背景：青色
    - ウィンドウ枠：金色
    - 背景：public/assets/img/character/hero/hero_fullbody.png を全画面に固定表示（透明度60%、オーバーレイ演出は既存と同条件）
...
- 概要
  ...
  - 初期入力値はカタカナで「セリア」とし、未入力のまま確定した場合も「セリア」を採用する
  ...
  - カタカナを横書きで並べてある
    ...
    - レイアウト
      ...
      - 文字
        ...
        - レイアウトは５文字ずつ揃える
  - 背景画像
    - 名前入力モードでは public/assets/img/character/hero/hero_fullbody.png を背景に固定表示する（オープニングと共通）
      - 透明度：60%
...
- 画面
  ...
  - ノード
    ...
    - 可視化ルール
      ...
      - Start から現在地点までの経路上にあるノードとエッジは常時表示し、既訪問ノードであることが視認できるよう彩度を落とした青系（例：#60a5fa）と淡いグロー（rgba(96, 165, 250, 0.45)）を付与する
...
- 操作
  ...
  - エンカウント演出
    ...
    - 会話モード初期描画時は以下の順にレイヤーを表示し、演出から自然に接続させる：
      ...
      3. 会話ウィンドウ
    - `route-canvas` より上位のフルスクリーンレイヤーに遷移エフェクトを描画し、画面全体を覆う。エフェクトの色調は暗緑〜紫のヴィラン系グラデーション（例：#0f172a→#312e81）を基調にし、中央から放射状に広がる光条と粒子を 0〜420ms で展開、420〜900ms でフェードアウトする。
      - エフェクト開始時（0〜180ms）は中央に収束した閃光を描画し、180〜420ms で画面全体へ拡散させる。
      - 420〜900ms では拡散したエフェクトをゆっくり減衰させつつ、会話モードのフェードインへ接続する。
    - 会話モード遷移時は、画面全体を不透明度0→1へ約220ms（ease-out）のフェードインで描画し、エフェクトから滑らかに接続させる。フェードイン中に HERO を左側から（x:-40px→0px, opacity 0→1, duration 260ms）、ENEMY を右側から（x:40px→0px, opacity 0→1, duration 260ms）スライドインさせ、それぞれ同時にアニメーションさせる。
...
- 表示
  ...
  - 主人公の画像
    ...
    - フレーム：幅570px × 高さ731px（ベース値）で表示し、必要に応じてレスポンシブに縮小する
      ...
      - 表示枠の背景色：黄色（例：#facc15）で暖色系のアクセントを付け、主人公側との色調差を明確にする
...
- 概要
  ...
  - ノードの位置別に相手キャラクターとお題テーマは異なるが、盤面サイズは HERO/ENEMY とも常に 5×5 に統一する。
  - 各バトルでは難易度ごとのピクロスを連続で解く。ピクロスバンクから重複しないよう抽選し、指定数すべてをクリアするとそのステージを突破したとみなす。
    - practice：2問
    - easy：4問
    - middle：6問
    - hard：8問
    - ultra：10問
  - 上記の合計は30問となり、勝利演出で表示するコレクションボード（30スロット）と一致させる。
  - 共通
    - BGM
      - 対戦中は `./assets/bgm/indie-game-pix-battle5.wav` を専用バトルテーマとしてループ再生する
  ...
  - レイアウト
    ...
    - 盤面上部に共通タイマー、ラウンド表示（例：Puzzle 1/5 や Puzzle 1/3）を表示する。
  ...
  - 勝敗と遷移
    - HEROが指定問数をすべて解いた場合は即座に勝利判定を行い、`ピクロスクリアーモード` へ遷移する。
    - ENEMYが指定問数をすべて解いた場合は、HERO敗北演出として `ENEMY勝利モード` へ遷移する。HERO側のスコア演出は停止し、敵の勝利ログを記録する。
    - HEROが誤答送信・残り時間0・リタイアを選択した場合は `ゲームオーバーモード` へ遷移し、敗北理由を表示する。
  - パズル生成
    - 各難易度ごとに「物語のモチーフを持つテンプレートバンク」を用意する。例：
      - practice：導きの妖精が描くきのこ・灯り・導きの矢印モチーフ（5×5）。
      - easy：森の動物たちやエモート（小さな笑顔・驚き・ハートなど）を描く 5×5。
      - middle：紋章・古代文字・封印石など物語のキーアイテムを象る 5×5 の複合テンプレート。
      - hard：精霊の守り手や森の試練を表す 5×5（大樹・森の守護剣・双月など）。
      - ultra：終章を象徴する神器・星図・森羅の円環など 5×5 の高密度テンプレート。
    - 各テンプレートは「記号化」用のメタ情報（`glyphId`, `glyphLabel`, `meaningText`, `collectionIndex`）を必須とし、勝利時のコレクションボード表示と連携する。
      - `glyphId`：内部一意キー（例：`practice_arrow`）。1ノード内で重複しない。
      - `glyphLabel`：コレクションボードに表示する1〜2文字のシンボル。モバイル表示でも潰れない字体を想定する。
      - `meaningText`：ストーリー上の意味（例：「導きの矢印」）。勝利画面およびコレクション詳細ツールチップで使用する。
      - `collectionIndex`：0〜29 の整数。難易度ごとに連番を割り当て合計30枠を埋める（practice:0-1, easy:2-5, middle:6-11, hard:12-19, ultra:20-29）。
    - テンプレートからパズルを生成する際は、物語用にデザインした 5×5 グリッドを「そのまま」出題する。左右反転・回転・オーバーレイ合成などで形状を変形させず、テンプレート定義時のシルエットを保持する。
    - 盤面はテンプレートの位置合わせ済みレイアウトを使用し、平行移動のみ許容する（中央揃え前提）。移動後も `glyphMeta.meaningText` に記載した象徴と視覚的に一致することを保証する。
    - 生成した盤面は `computeClues` でヒントを算出し、バックトラッキング型ソルバで唯一解かを検証する。唯一解でない場合はテンプレート定義を修正するか除外する。
    - HERO / ENEMY 双方で同一のテンプレート集合を共有し、1バトル内では `collectionIndex` 昇順に出題する。プレイヤー視点でストーリー順に記号を収集できるよう、出題順のシャッフルは行わない。
    - 生成ロジックは乱数シードを引き続き受け付けるが、テンプレート選択順は固定のため seed は主にデバッグ目的のログ出力に限定利用する。
    - 返却値は各パズル毎に `grid` と `glyphMeta`（上記4プロパティ＋後述の `difficultyTag`, `collectionOrdinal` などの拡張フィールド）を含め、UI 層が勝利演出に即座に利用できる形にする。行列ヒントはUI側で `computeClues` を用いて算出する。
...
- 画面
  - 初期表示
    ...
    - 画面下段に配置していたスペルログ（メッセージエリア）は廃止する
      - スペル発動時の履歴一覧は表示せず、各キャラクターの吹き出し演出のみを用いる
      - ログ領域の削除後は下段に余白を残さず、既存の盤面・パネル配置を維持する
  - 背景画像
...
- 操作
  ...
  - スコア加算演出
    ...
    - ステージクリア時は、BGMフェードアウト完了後に勝利SE `./assets/bgm/indie-game-fanfare.wav` を1度だけ再生する
      - 既に勝利SEが再生中の場合は停止してから先頭から再生し直し、ループはさせない
      - 勝利SEの再生ゲインは既存のスコア加算効果音より控えめ（通常の約75%）とし、他サウンドを覆わない
...
- 内部データ・状態
  ...
  - ピクロス終了状態
    ...
    - 終了時のピクロスの状態と回答が一致してい無い場合
      ...
      - ゲームオーバーモードに移行する
  - コレクション進行管理
    - 各パズルをクリアした瞬間に `glyphMeta.collectionIndex` をキーとしてローカル進行データにフラグを立て、1ステージ内で重複して出題されても再加算しない。
    - 進行データは `localStorage` の `picrossGlyphCollection`（JSON）に永続化する。キーは難易度・キャラクター固有IDを含め、セーブデータの削除（New Game）で初期化する。
    - コレクション情報はルートマップ画面に戻った際も保持し、勝利演出だけでなくバトル準備UIからも進捗を参照できるようにしておく。
    - 各コレクション解放時には、テンプレートの正規化済みグリッド（回転・合成無しの素体）と `unlockedAt`（ISO8601文字列、例：`2025-08-10T12:34:56.789Z`）を保存する。勝利画面では `unlockedAt` を `YYYY/MM/DD` 表記に整形して表示する。
...
- 表示
  - 背景
    - 画面全体に `public/assets/img/background/win_bg.png` を中央寄せ・coverで敷き、既存のグラデーションオーバーレイ（rgba(15,23,42,0.65) など）を重ねてテキストやポートレートの視認性を確保する。
    - 背景はモード開始時にフェードインし、次モードへ遷移する際は200msでフェードアウトする。
  - レイアウト
    - デスクトップ幅では左右2カラム（左40%：HEROセクション、右60%：コレクションボード）に並べ、`gap: 40px` 程度を確保する。
    - モバイル幅（≤768px）では縦積みに切り替え、HEROセクション → コレクションボード → ボタンの順で中央揃えにする。
  - HEROセクション（左カラム）
    - ポートレートフレーム：幅380px × 高さ487px（会話モードと同値）。768px以下は幅300px、480px以下は幅240pxに縮小し縦横比を維持する。
    - フレーム内で画像は object-fit: cover で表示しつつ、カード中央に水平・垂直方向ともセンタリングする。背面のグロー演出は左右非対称にならないよう調整する。
    - 勝利タイトル：`<HERO_NAME> WIN` をポートレート下に左揃えで表示し、サブテキストで今回獲得したシンボル名（例：「新たな記号：導きの矢印」）を1行表示する。複数解放した場合は `、` 区切りで並べ、「新たな記号：導きの矢印、森灯の火種」のようにまとめる。
  - コレクションボード（右カラム）
    - 木目調の四角いボードパネル（推奨比率 4:3、最小幅 420px）を描画し、内部のスロット数は難易度ごとの実際のコレクション総数に合わせる。
      - PRACTICE：2 / EASY：4 / MIDDLE：6 / HARD：2 / ULTRA：3（合計17スロット）
    - 各スロットは丸みを帯びたソケット（直径72px想定）で表現し、未解放時は枠のみ（透過度60%）を表示、解放後に記号と発光リングを表示する。
    - 難易度ごとにスロットへ固定割り当てを行い、practice→easy→middle→hard→ultraの順で左から右、上から下に埋める。カーソル／タップ時に `glyphMeta.meaningText` をツールチップで表示する。
    - 今回新たに埋まったスロットは 0.6s のポップアップアニメーション（スケール1.2→1.0、アルファ1.2→1.0）を再生し、既存のスロットは静的状態で表示する。
    - サイズ別ボード構成
      - 5×5 用ソケット群（PRACTICE〜MIDDLE）は 12 スロットに合わせ、5列×3行を基準に必要行数のみで構成する（余白の空スロットは作らない）。右カラム上段に配置し、ボードタイトルを「Elfpix Symbol 5×5」にする。
      - 10×10 用ソケット群（HARD / ULTRA）は 5 スロットに合わせ、5列×1行を基準に必要行数のみで構成する。右カラム下段に独立したボードを用意し、タイトルは「Elfpix Symbol 10×10」とする。
      - モバイル（≤768px）時は 5×5 ボード → 10×10 ボードの順で縦積みし、各ボード間に 16px の `gap` を確保する。デスクトップでは上下配置を維持しつつ、５×５／１０×１０ ボードとも右カラム幅一杯を使用する。
      - 10×10 スロットは 5×5 と同じ情報表示（記号、難易度タグ、アンロック演出）を共有するが、セル内のフォントサイズを 85% に縮小し、格子幅を 64px 程度に調整する。
      - アンロック順序は HARD → ULTRA の出題順を左から右、上から下へ配置し、ステージ番号のサフィックスは `H-01` 形式（H=HARD/U=ULTRA）とする。
  - 記号ピース表示
    - `glyphLabel` をスロット中央に大きく描画し、下部には小さめの英数字で難易度タグ（例：`EASY-03` ※難易度ごとの1始まり連番をゼロ埋め2桁で表示）と解放日付（`YYYY/MM/DD`）を表示する。
    - すべてのスロットが埋まった場合は、ボード右上に「COMPLETE!」リボンと祝福エフェクト（星粒が周囲に散る演出）を表示する。
  - Continueボタン
    - レイアウト下部右寄せに配置する（モバイルでは全幅ボタンとして中央揃え）。クリック時にアニメーション演出が完了していなくても次モードへ遷移できる。
- サウンド
  - 勝利演出に入った瞬間に勝利SE `./assets/bgm/indie-game-fanfare.wav` を非ループで再生し、演出終了まで自動で減衰させる
    - 勝利SE再生中にサウンド設定をOFFへ切り替えた場合は即時停止する
  - 新規記号が解放された場合は、SE `./assets/se/collection_unlock.wav` を 1 回鳴らし、すでに解放済みのスロットのみの場合は再生しない。
- 操作
...
- 概要
  - ENEMY側が指定問数を先にすべて解いた際に表示する敗北演出モード。
...
- 操作
  ...
  - キーボードショートカットはゲームオーバーと同様に Enter / Space = Continue、Esc = Quit を受け付ける。

### ゲームオーバーモード

- 概要
  - 残り時間が0になる、または誤答送信・リセットによる敗北時に表示する標準のゲームオーバーモード。
  - 画面は暗転背景に `Game Over` を表示し、敗北理由を短いテキストで補足する。
  - Continue/ Quit の選択肢を提示し、次のアクションを決定させる。
