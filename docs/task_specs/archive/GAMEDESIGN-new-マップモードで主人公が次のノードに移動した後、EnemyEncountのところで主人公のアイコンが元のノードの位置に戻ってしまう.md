...
### タスク仕様（マップモードの主人公アイコン位置維持）

- ENEMY ENCOUNT 演出中／演出完了後も、主人公アイコンは「移動先ノード」に固定して表示する。
- エフェクト再生や遷移準備のタイミングで `current` を元ノードへ戻さず、移動先ノードを現在地として扱う。
- `pendingNode` など遷移用の状態を使う場合でも、マップ表示は到達済みノードを参照する。
- Enemy Encount の吹き出しや演出は到達ノードに紐づけて表示し、演出終了後も位置が戻らないこと。
