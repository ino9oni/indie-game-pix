...
- エンドレス一人プレイ用のモードを追加
  - 背景・目的
    ...
    - CPU・ENEMYと対戦せずに自分のペースでピクロスだけを楽しみたい人用のモードを用意する
  - 作成日
    - 2025/11/22
  - 追加したい箇所や場所
    - オープニング画面
  - 仕様
    ...
    - その下にエンドレスモードを追加する
      - エンドレスモード
        ...
        - 成り立つ前提の動的にお題を生成し、その場でプレイヤーに解いてもらう
        - ユーザがQuitを推せば、ゲームを終了し、エピローグ画面まで戻る

- 一人用の導入
  - 一人でプレイするモードの導入
  - 対戦ではなく一人で淡々とelfpixを説くだけのモードでも楽しめる
...
- エンドレスモードの導入
  - SOLVERを使いながらアドホックなお題を生成して、いくらでもピクロスを遊ぶ事が可能なモードの導入
  - ストーリーモードとは違い、ピクロスボードのサイズを30x30程度まで、お題は∞（やめるまで）、or　お題数指定で拡張できるようにする
  - スコアについてはハイスコア程度の記録としてオープニングとエンディングに残すようにする
