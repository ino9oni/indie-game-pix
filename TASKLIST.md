...
- プロローグの表示方式の修正
  - ウィンドウサイズの縦幅がやや大きいため、現在の高さの75%程度にする
- 名前入力画面の不具合修正
  - 名前入力画面で、ボタンを右端に寄せたことで、カタカナの並びが崩れたので、ウィンドウサイズを広げて文字は５文字ずつ、５行ずつの表示にする
...
- スコア形状の音の変更
  - スロットマシン
- ピクロスクリア時のファンファーレをよりメロディアスおよびゲームクリア―感を出す
- ピクロスモードのクリック時の効果音の変更
  - 金属音ではなく、ガラスをたたく音もしくは、水のような音、もう少し優しい感触の音にする
  - クリックした時に、エフェクトを入れる
    - 魔法を使って発掘したようなエフェクトを入れる
- マップモードの変更
  - マップモードのノードクリック時に、キャラクターの画像を移動元から移動先までエッジに沿って、２秒程度で移動させる
  - 移動終了後に、ノードの上に「Enemy Encount！」を小さい吹き出しで表示する
  - そのあと会話モードに移行する
- [done] ~~エンディングの追加~~
