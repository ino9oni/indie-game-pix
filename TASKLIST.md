...
## Priority: high
- [done] マップモードでストーリーのレイアウトを左側に配置する
  - 背景・目的
    - 右の方はマップの操作と逆方向に表示されるため連なりが感じづらいため、ストーリーの表示エリアを左側に設置する
  - 作成日
    - 2025/12/27
  - 追加したい箇所や場所
    - マップモード
...
- [done] マップの背景が真っ黒で冒険感がないため、マップの背景を表示し、マップのオブジェクトに合わせて進行できるようにする
  - 背景・目的
    - マップ背景が黒一色で冒険感が薄いため、背景を表示し進行の視認性を高める
  - 作成日
    - 2025/12/27
  - 追加したい箇所や場所
    - マップモード
  - 仕様
    - マップ背景として assets/img/background/map.png を固定表示する
    - 背景のランドマークに合わせてノード/エッジの位置を調整する
    - 既存のズーム/スクロール/エンカウント演出は維持する
...
- マップモードで主人公が次のノードに移動した後、EnemyEncountのところで主人公のアイコンが元のノードの位置に戻ってしまう
  - 作成日
    - 2025/12/27
  - 事象・問題点・仕様と異なる点
    - ENEMY ENCOUNTした位置にとどまるがとどまらず、エフェクトが発生した後に元のノード位置に強制的に戻されているように見える
  - 不具合の箇所・機能・大まかな箇所（UI/ロジック）
    - マップモード
  - 改修案、改修例を記載
    - ENEMY ENCOUNTした位置にとどまる
  - 不具合の重要度
    - 影響度：小（理由：ゲームは進行できるため）
    - 発生頻度：大（理由：常に）
...
- [done] ENEMYのスペルの効果でスペル発動風の挿絵がHERO側を向いていない
  - 作成日
    - 2025/12/23
  - 事象・問題点・仕様と異なる点
    - 明確な仕様がないが表題の通り
  - 不具合の箇所・機能・大まかな箇所（UI/ロジック）
    - ピクロスモードのジャマ―発動時
  - 改修案、改修例を記載
    - ENEMYのスペルの効果でスペル発動風の挿絵をHERO側を向くように反転
  - 不具合の重要度
    - 影響度：小（理由：進行上問題ないため）
    - 発生頻度：中大（理由：ジャマ―発動時のみ表示のため）
...
- [done] ヘッダのメニューの画面上部のPaddingが入っていないので入れる
  - 背景・目的
    - ヘッダのメニューが画面上部にくっついてしまっているため、気持ち悪い、パディングを入れる
  - 変更前の仕様：
    - 現行
  - 変更後の仕様：
    - パディングを気持ち入れる（10px程度）
...
## Priority: normal
- ヘッダのメニューの画面上部のPaddingが入っていないので入れる
  - 背景・目的
    - ヘッダのメニューが画面上部にくっついてしまっているため、気持ち悪い、パディングを入れる
  - 変更前の仕様：
    - 現行
...
- [done] エンディングのクレジットで、イラストは「Stable Diffusion」ではなく「ChatGPT」に変更
  - 背景・目的
    - クレジットの名称を変更したい
  - 変更前の仕様：
    - 現行仕様
...
- [done] マップモードのStart時点の初期表示のストーリーはプロローグとスタート時点のストーリーを表示しておく
  - 背景・目的
    - Startした直後にストーリーがストーリーブロックに表示されていないのでプロローグとStart押してからのストーリーを入れておく
  - 変更前の仕様：
    - ゲーム開始後のマップ画面の初期表示にストーリーブロックが何も表示されていない
...
- [done] 画像や音楽のダウンロードに時間がかかっていて処理が重いので最初に全部ダウンロードしておきキャッシュし高速化する
  - 背景：ゲームプレイ中に画像や音楽のダウンロードを行うとフリーズするため、事前にリソース情報をダウンロードしておきたい
  - 改善内容：画像や音楽のリソース群をあらかじめダウンロードし、ダウンロードしたものを利用することでユーザレスポンスを高速化する
  - 仕様：
    - ダウンロードのタイミング
...
- [done] マップモードの画面右にここまでのストーリーを表示する
  - 背景・目的
    - プレイ後にストーリーを失念しやすいため
    - ストーリー記述を整理することで、感情移入やプレイ没入感を向上させるため
  - 変更前の仕様：
    - なし
  - 変更後の仕様：
    - 表示のスタイルは会話モードのスタイル風にする
    - レイアウト
      - マップモードの幅は画面の7.5割
      - ストーリーの表示幅は画面の2.5割
      - マップモードの幅によりマップが折れ曲がったりしないようにサイズとエッジの長さは調整し可能な限り収まるようにする
    - ストーリーの表示
      - あらましとしてのストーリーを記述する
        - ENEMYとの闘いの様子と結果
        - 次の場所の解説
        - 次の場所へ移動する目的
      - ENEMYを倒すごとにストーリーが拡充されていく

- [done] デバッグモードの改善
  - 背景・目的
    - デバッグモードでデバッグしたい機能が足りていないので追加する
  - 変更前の仕様：
    - 現実装
  - 変更後の仕様：
    - ENEMYのコンボスペルを現状から１上げる
    - ENEMYのコンボ数をMAXにする
    - ENEMYがお題の９割をクリアした状態にする
    - ENEMYがお題の現在の回答済みお題に加えて１つクリアした状態にする（次のお題に進める）
    - HEROのコンボスペルを現状から１上げる
    - HEROのコンボ数をMAXにする
    - HEROがお題の９割をクリアした状態にする
    - HEROがお題の現在の回答済みお題に加えて１つクリアした状態にする（次のお題に進める）

- [done] エピローグのクレジット画面に「Special Thanks」として「sideofall」を追加する
  - 背景・目的
    - スペシャルサンクスの人を追加する
  - 変更前の仕様：
    - 省略
  - 変更後の仕様：
    - タイトルは「Special Thanks」
    - ロールは「TestPlayer」
    - 名前は「sideofall」
...
- [done] Reset,Submit,Quitボタンを非表示にする
  - 背景・目的
    - 特に利用しなくなったため
  - 変更前の仕様：
    - 常にReset, Submit, Quitボタンが表示されている
  - 変更後の仕様：
    - 常にReset, Submit, Quitボタンを非表示にする
    - PauseボタンをPause/Quitボタンに変更する
    - Pause/Quitボタンを押下すると画面にPauseがかかり、Pause中の画面に「Quit」ボタンを表示する
    - キーボードの「Escキー」を押下すると同様のPauseがかかり、Pause画面を表示し、同様とする

- [done] マップモードのアイコンのサイズを２倍にする、エッジも道に見えるように太くする
  - 背景・目的
    - 小さすぎて見えづらいため
    - キャラクターのミニチュア感がないため
    
- [done] ENEMY側のスペルが発動が少ないの発動できる場合は発動するようにする
  - 背景・目的
    - ENEMY側からスペルの影響を受ける機会が少ないまたはほぼないのでこれを改善したい
  - 変更前の仕様：
    - ENEMYのコンボカウントが溜まったら、ENEMYがジャマ―を発動しない
  - 変更後の仕様：
    - ENEMYのコンボカウントが溜まったら、ENEMYがジャマ―を発動する
    - ENEMY側も解答やMAYBEが正しく合っていればコンボカウントを計上する
    - ジャマ―の攻撃種類帯としては、レベル１－３まで程度とする

- [done] 勝利画面のコレクション数が実際のコレクション数とあっていないので修正
  - 背景・目的
    - 勝利画面のコレクション数の枠が、すべての敵のお題のコレクション数と合致していない可能性がある
  - 変更前の仕様：
    - 現実装
  - 変更後の仕様：
    - コレクション数をただす
      ...
      - HARD：２個
      - ULTRA：３個
    - また敵を倒す前には枠だけが表示されており、クリアしてから枠に対象が表示されるようにする
      - 枠の数だけ残っている状態にする

- [done] プロローグ画面では画面上部の画面共通のヘッダを表示しない
  - 背景・目的
    - プロローグ画面で画面上部の画面共通ヘッダが表示されるとプロローグ感が減少するのでやめる
    - プロローグ画面で画面上部の画面共通ヘッダが表示される場合に、BGMをオフにできてしまうが、そのあとオープニング画面で再度ボタンを押下すると再度BGMが再生始まりバグのように見えるので、最初は表示しないようにしたい
  - 変更前の仕様：
    - 現実装
  - 変更後の仕様：
    - 背景・目的に準ずる
    
- [done] ストーリーの非表示ボタンを押下した際に、非表示になった分マップを広げて非表示に意味を持たせる
  - 背景・目的
    - 非表示ボタンを押下しても、マップがそのままで、ストーリーだけ非表示されて非表示に意味がない状態
  - 変更前の仕様：
    - 現実装
  - 変更後の仕様：
    - 非表示ボタンを押下すると、ストーリーエリアを非表示にする
    - 非表示ボタンを押下した後は、再表示ボタンに変化するが、再表示ボタンは画面の右端にしおりのような形で縦表示する
    - ストーリーエリアを非表示にした後は、マップエリアを非表示になった幅分広げる（再表示した場合は、元の幅に戻す）
...
- [done] MIDDLEの分岐をやめて、PRACTICE-＞EASY-＞MIDDLE-＞HARD->ULTIMATEの順番にする
  ...
  - 変更前の仕様：
    - 現実装
  - 変更後の仕様：
    - EASY-＞MIDDLE-＞HARDの順でその前後は同様とする
    - STARTから右上に進行するようにするが５つ連なるためエッジの長さを少し短くしてすべてのパスが収まるようにする（エッジが折れ曲がらないようにする）
    - ストーリーはつながるように、前後関係の整合性を取るようにする

- [done] マップモードのキャラクターを４倍のサイズにして視認性を高める、加えて、エッジの色を土色にして線幅太くしてこちらも視認性を高める
  - 背景・目的
    - キャラクターが小さすぎて視認性が悪いので、大きくする
    - エッジが細すぎて道感がないので、道っぽくする
  - 変更前の仕様：
    - 現実装
  - 変更後の仕様：
    - キャラクターの表示PNGのサイズを縦横４倍にします。
    - 園児の色を土色にする。太さを10pxにする。

- [done] Hardモード以降の10x10のピクロスモード時の決定的ヒントが少ないのでこれを改善する
  - 背景・目的
...
- [done] 会話モードのキャラクターの表示領域の背景を半透明にする
  - 透過率:60%
  - 対象:HEROと全ENEMYの両方
