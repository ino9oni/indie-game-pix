...
### タスク仕様（ENEMYスペル演出の向き調整）

- 目的：ENEMYのスペル発動演出で表示される挿絵がHERO側を向くように統一する。
- 対象：ピクロスモードのスペル発動シーン（ENEMY側の発動時のみ）。
- 変更内容：
  - ENEMYのスペル発動中に表示する挿絵は、腕がENEMY側からHERO側へ伸びる向きにし、エネルギー弾（円）の位置はENEMY側に残す。
  - HERO演出やボード攻撃エフェクトの向きには影響させない。
