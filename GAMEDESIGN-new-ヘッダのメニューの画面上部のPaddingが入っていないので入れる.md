...
## タスク仕様（ヘッダのメニューの画面上部Padding追加）

- 画面上部のヘッダメニューが天井に密着しないよう、トップアクション領域に上側パディングを追加する。
- 目安は 10px 程度とし、既存のボタン高や余白バランスを崩さない範囲で調整する。
- 既存の横並びレイアウト・ボタン配置は維持し、画面幅ごとの配置崩れを起こさない。
- ヘッダ非表示の画面（プロローグ等）には影響しないようにする。
