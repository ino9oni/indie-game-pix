...
- スペルボタン押下と同時にピクロス盤面への入力・カウントダウン・ENEMY AI を 900ms 停止し、盤面全面に暗転レイヤー（opacity 0→0.8, ease-out 180ms）を適用する。暗転中はセル操作・Pause ボタン・デバッグメニューを無効化する。
- 暗転に合わせてキャラクター（HERO/ENEMY）の怒り立ち絵を左右から `translateX(±60px)` ＋ `opacity 0→1`（280ms）でスライドインさせ、中央寄りにスペル名＋セリフを表示する吹き出しを 1.2 秒表示する。吹き出しテキストは `audio.playSpellSpeech()` と同期させる。
- 盤面上に CSS / WebGL ベースの 3D 風エフェクトレイヤーを 1 秒間重ね、難易度ごとに色調・モチーフを切り替える（PRACTICE=光、EASY=水、MIDDLE=落石、HARD=毒霧、ULTRA=炎）。レイヤーは `mix-blend-mode: screen` でボードに馴染ませる。
- スペル効果（ジャマー、セル塗り戻し、ロックなど）のロジックは 500ms 以内に実行し、暗転解除時点で反映済みの盤面状態を表示する。効果発動中も HUD のコンボ数・ログは更新する。
- 1.2 秒経過後に暗転・立ち絵・3D レイヤーを `opacity 1→0`（200ms）でフェードアウトし、`resumeBoard()` を呼び出してタイマー・AI・入力を再開する。フェードアウト完了まではセル操作不可とし、解除と同時に通常プレイへ戻る。
- HERO 側スペル時は、怒り立ち絵ではなく全身ビジュアルを左側から画面中央 40% 程度までスライドインさせ、上半身が切れない構図で表示する。ENEMY 側は従来の怒り立ち絵を右側から表示する。
- ジャマー適用後、敵盤面上に 0.8 秒間の 3D 攻撃エフェクト（炎・水流・落石など）を表示し、影響マスにパーティクル／衝撃波が走る演出を加える。エフェクトは Three.js 等の 3D レイヤーでレンダリングし、攻撃対象セル付近に配置する。
- HERO 側・ENEMY 側ともに、スペル演出中はキャラクターを画面高さいっぱい（高さ 90vh 目安）で描画し、背景までを覆うフルスクリーンのシルエットとして表示する。重なり部分は暗転レイヤー上に配置し、見切れが発生しないようにする。
- 演出の順序は「キャラクター＋台詞表示 → 0.5秒の静止 → 敵盤面へフォーカスした攻撃エフェクト → 元画面へ復帰」とし、敵盤面のアップ時にはピクロスボードのみを強調表示して攻撃瞬間を視認しやすくする。
