...
- 画面
  - カタカナを横書きで並べてある
    ...
    - レイアウト
      ...
      - 入力ウィンドウ全体の幅は従来より広げ、最大1040px（最小幅は端末幅に応じて縮小）とする
      - 各行は必ず5文字ずつ横並びにし、5行構成が崩れないようマス目幅を均等にする
      - 空白セルはボタンを非活性表示とし、レイアウトのみ保持する
...
- 操作
  - 操作パネル
    - 操作ボタンは入力ブロック右端に縦並びで配置し、決定ボタンが最下段に来るよう固定する
  - 決定ボタン
    - 主人公の名前を"入力値"とする
...
- 表示
  ...
  - 会話のウィンドウ
    ...
    - 敵
      ...
      - ウィンドウの内部：セリフ本文
    - テキスト表示
      - プロローグと同様に、テキストの切り替え時は既存テキストを200msでフェードアウト→新テキストを200msでフェードインする
      - 話者名・本文とも同じアニメーションタイミングで切り替える
...
- 概要
  ...
  - 共通
    ...
    - スコアシステム
      - スコアはゲーム全体で共有される累積値とし、ピクロスをクリアするたびに加算される
      - 初期値は0点とし、NewGame時・ゲームオーバー時・エンディング終了時に0へリセットする
      - 各ピクロス開始時点では直前までの累積スコアを保持したまま表示する
...
- 画面
  - 初期表示
    - 残り時間表示バー
    - スコアパネル
      - 表示位置：画面左上（ヘッダー直下）のHUDに配置し、残り時間表示と横並びで常時表示する
      - 表示内容：ラベル「Score」と6桁ゼロ埋めの合計スコアを組み合わせて表示する（例：Score 012345）
      - 通常時は現在の累積スコアを静的に表示し、加算演出中は数値のみ高速にカウントアップする
    - ピクロス画面
      - 難易度にあわせてピクロスを表示
...
- 操作
  - セル操作
    - マスを塗りつぶす／バツ印を付ける操作時に、石を削り出すような短い効果音（assets/se/chisel_tap.wav）を再生する
  - **Submit** を押下する
    - 正答の場合はスコアを計算して累積値へ加算し、ステージクリアのダイアログは表示せずに次のモードへ遷移する
      ...
      - スコア加算演出開始時に8ビットゲーム風の効果音（assets/se/score_chiptune.wav）を再生し、演出終了までループ／フェードアウトさせる
    - 不正解の場合はゲームオーバー処理へ遷移する（スコア加算は行わない）

  - クリア後の演出
    - スコア加算演出の完了後に、メロディアスでゲームクリア感のあるファンファーレ（assets/se/fanfare_melodic.wav）を再生する
    - ファンファーレ再生中は祝福感のある画面演出を表示し、演出終了後に次のモードへ遷移する
  - **Submit** を押下する
...
- 遷移
  ...
  - タイトルへ遷移する際に累積スコアを0へリセットし、Score表示を初期化する。
...
- 操作
  - **Continue** ボタンを押下
    ...
    - ゲームの時間を20分に戻す
    - スコアは直前までの累積値を保持し、再挑戦後のクリア時に加算演出を再度実施する
  - **Quit** ボタンを押下
    - ゲームオーバーの音楽を流す
    - ゲームオーバーである旨を表示する
    - 累積スコアを0へリセットし、タイトルへ戻った時点でScore表示を0へ更新する
