...
### タスク仕様（HERO側のスペル発動時にスペルの効果イメージが表示されない）

- 目的：ENEMY 側では表示されているスペル効果イメージが HERO 側で表示されない不具合を解消し、演出の整合性を確保する。
- 対象：ピクロスモードの HERO スペル発動時シネマティック／ボード攻撃演出。
- 変更内容：
  - HERO スペル発動時にも属性に応じた効果イメージを表示する。演出レイヤーは左側からスライドインし、ENEMY 側と干渉しない別レイヤーを使用する。
  - 演出フロー：暗転 → HERO 全身＋台詞表示 → HERO 用効果イメージを重ねる → 相手ボードへの攻撃エフェクトを適用 → 画面を復帰、の順で ENEMY と同じテンポを維持する。
  - 効果イメージのアセットは ENEMY 用と同等解像度で用意し、HERO 用の属性マップを追加して選択できるようにする。
  - ミュート／演出スキップが有効な場合はイメージ表示を省略するが、盤面処理タイミング（ロック・攻撃・進捗更新）は ENEMY と同一に揃える。
  - 演出の有無にかかわらず、ボード処理（ロック、攻撃効果、進捗更新）は HERO／ENEMY で同一タイミングに適用し、演出とロジックのズレを防ぐ。
