...
- 背景
  - 画面全体に `public/assets/img/background/tutorial_bg.png` を中央寄せで敷き、常時カバーさせる。
  - 背景画像の上から濃紺のグラデーションオーバーレイを重ね、文字や盤面が読みやすいコントラストを確保する。
- 既存仕様との整合
  - 左カラムの解説キャラクター表示や右カラムのピクロス盤面、下部の説明ウィンドウは従来レイアウトを維持する。
  - BGMや操作シーケンス、チュートリアル進行ロジックに変更は加えない。
