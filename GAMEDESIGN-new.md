...
### タスク仕様（ピクロス回答時のバーが1問で100%になる不具合）

- 目的：進捗バーが実際のクリア件数に比例して増えるよう正す（1問正解で100%にならないようにする）。
- 対象：ピクロス対戦画面の進捗バー（HERO/ENEMY 両方）。
- 変更内容：
  - 進捗計算を「クリア済みお題数 / 目標お題数」に統一し、1問クリア時は `1 / 目標数` の割合で表示する。
  - ENEMY側も同じ計算式を使い、解答中のセル数ではなく達成クリア件数ベースにする。
  - 進捗バー表示ロジックとUIが同じ定数・関数を参照するよう整理し、難易度やノードで目標数が変わっても正しく追従するようにする。
