...
- マップモードの表示領域（`route-canvas`）はビューポート基準で幅・高さとも 70% を目安にしつつ、`min()` や `clamp()` を用いて最小 960×640px、最大 1200×900px に収める。ウィンドウリサイズ時は中央起点で再配置し、上下左右に 24px 以上の余白を確保する。
- ノード座標は正規化した比率（0〜1）で保持し、新しいキャンバス寸法に応じて算出する。これにより表示領域のスケールが変わってもノードが画面外へはみ出さない。
- ノード間エッジは各接続の進行方向に合わせて角度 40°（±2°許容）の斜め直線で描画する。SVG ラインの `stroke-linecap="round"` を使用し、ノード半径を考慮した開始・終了座標を算出する。
- 角度固定に伴い、エッジが重なる箇所には 4px 程度のオフセットを与えて重なりを軽減する。ホバー／フォーカス時はノードとエッジのハイライトが衝突しないよう Z-index を調整する。
- マップ背景のグリッド・ブラーを新しいスケールに合わせて再描画し、視認性を保つ。小さな画面（幅 < 1280px）では背景を 60% 透過させノード群のコントラストを確保する。
- `RouteMap` をゲームパッドで操作する際のカーソル挙動、デバッグメニューのマップジャンプ処理が既存と同じ結果になることを確認し、必要であれば座標変換を共有化する。

### タスク仕様（マップモード描画のエッジ描画の角度を変更）

- マップモードのノード間エッジは進行方向に沿って角度 30°（±2°許容）の斜め直線で描画する。現行の 40°実装を差し替え、単一の直線セグメントでノード中心同士を結び、ノード半径を考慮した開始・終了座標を再計算する。
- 角度変更によりエッジがノードのラベルや他エッジと干渉しないよう、必要に応じて 4px 前後のオフセット・Z-index・ストローク幅を調整し、ラベルの可読性と操作性を維持する。
- エッジ角度の再計算はリサイズ時やデバッグジャンプ時にも同期されるよう、座標正規化ロジックと共通の変換を使う（ユーティリティ化を検討）。
- マップキャンバスは横幅 100%（`width: 100%` 相当）で表示し、縦方向は `clamp()` を用いて最小 640px、最大 900px の範囲に収まるよう調整する。既存の固定幅指定を撤廃する。
- 角度変更後もヒーロー移動アニメーション、ゲームパッド移動、アクセシビリティ属性、デバッグメニュー経由ジャンプが既存通り動作するか回帰を確認する。

### タスク仕様（マップモードのキャンバスサイズの変更）

- `.screen.route` の `max-width` 制約を撤廃し、ビューポート幅に対してフル幅（100%）でマップ領域を表示する。上下余白は既存の `padding` を踏襲しつつレスポンシブに調整する。
- ルートマップの開始ノード（`start`）は画面下段寄りに配置し、ビューポート高さに応じた基準で Y 座標を再計算する。既存のノード正規化処理を拡張し、配置後もはみ出しが発生しないよう補正する。
- ノード間の接続距離（水平ステップ）を現状のおよそ 75% に縮小し、全体のマップ縮尺を引き締める。距離縮小に伴うエッジ角度（30°）の整合性とアニメーション速度の再調整を行う。
- キャンバスの高さは `clamp()` により最小 640px、最大 900px、推奨 70vh を目安として維持し、幅方向は 100% 表示に合わせて再算出する。
- 操作・演出（ゲームパッド移動、デバッグジャンプ、ヒーロー移動アニメーション、アクセシビリティ）が従来通り機能することを確認する。
