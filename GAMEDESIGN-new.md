...
### タスク仕様（HEROのスペル（ジャマ―）の効果が相手に入った際に、回答ステータスバーの状態が変化しない）

- 目的：HEROスペル（ジャマ―）による手戻りで相手盤の進捗バーが更新されない不具合を解消し、進捗表示と勝敗判定の整合性を取る。
- 対象：ピクロスモードの進捗バー計算・表示ロジック（特にENEMY側の進捗更新／クリア判定）。
- 変更内容：
  - HEROスペルで相手盤のセルを初期化・減算した直後に、相手側進捗を必ず再計算するフックを追加し UI を即時更新する。
  - 進捗計算は「解答と一致する1セル数 ÷ 解答総塗りセル数」で毎回再計算し、`?` や誤答セルは分子に含めない。
  - 進捗減算に伴い、クリアフラグや解決済みフラグをリセットして再評価することで、誤ったクリア扱いを防ぐ。
  - HERO/ENEMY いずれのスペルでも同じ進捗更新手順を共有し、ミュート／演出スキップ時も処理タイミングは共通とする。
  - バー表示が急激に跳ねないよう、再計算結果を滑らかに反映する（差分に応じたアニメーション）。 
