...
- 表示
  - 主人公の画像
    ...
    - フレーム：幅380px × 高さ487pxの表示枠に収め、object-fit: contain で中央配置
  ...
  - practice
    ...
    - 対戦相手
      ...
      - 属性
        ...
        - 画像（通常）：assets/img/character/enemies/00011-3639403992.png
        - 画像（笑顔）：(test画像)
        - 画像（怒り）：(test画像)
  - easy
    - ノードとの対応
...
- マップモードの変更（拡大・中央配置）
  - ルート画面のキャンバスを画面高に応じて拡大（min-height: calc(100vh - ヘッダ相当)）。
  - SVGの高さを viewport 高の約75%に設定し、中央に配置。
  - 既存の座標系（viewBox: 800x400）は維持し、スケールはブラウザに委ねる（object-fit 相当）。
