...
### タスク仕様（Hardモード以降の10x10ピクロスで決定的ヒントを補強する）

- 目的：Hard/Ultra（10×10）で決定的ヒントが不足し、序盤の足場が少ない問題を解消する。
- 対象：10×10 盤を生成するハード以上のテンプレート／正規化処理。
- 変更内容：
  - 行・列の 10〜20%（目安として各方向2本以上）に「決定的ヒント」パターンを必ず含める。候補: `[1,1,1,1,1]`, `[5,4] / [4,5]`, `[3,3,1] / [1,3,3] / [3,1,3]`, `[2,2,4] / [4,2,2] / [2,4,2]`, `[6,3] / [3,6]`, `[7,2] / [2,7]`, `[8,1] / [1,8]`。
  - 決定的ヒントを優先的に挿入した上で、既存のヒント分布（10:10% / 7–9:10% / 4–6:40% / 2–3:30% / 1:10%）を崩さないよう再正規化する。
  - 生成後は一意解チェックを必須とし、決定的ヒント挿入で解の非一意化が発生した場合は再生成／再配置を行う。
  - 同一パターンの連続や左右上下対称による単調化を避けるため、決定的ヒントは行・列で分散させ、反転一致が過半に達した場合は再配置する。
  - 再生成が一定回数失敗した場合は、情報量スコアの高い候補を採用しつつ「決定的ヒントを最低1本以上含む」条件を保持する。

### タスク仕様（ENEMY側のスペル発動頻度を改善する）

- 目的：ENEMY のコンボが溜まってもスペル（ジャマー）がほとんど発動しない問題を解消し、対戦バランスを向上させる。
- 対象：ピクロスモードの ENEMY コンボ蓄積・スペル発動ロジック。
- 変更内容：
  - ENEMY のコンボカウントを「正答セル」「正しい ?（未確定）入力」を含めて蓄積し、閾値到達時に確実にスペル発動キューへ積む。
  - 発動判定は時間経過またはコンボ到達で即チェックし、行動ロック中やボードロック中は解除後に最優先で実行する。
  - 発動タイミングの上限待ち時間を短縮（例: 1.5〜2.5s 以内に必ず発動）し、連続発動はクールダウンを設ける（レベル1〜3程度の攻撃種別に収める）。
  - HERO 側と同じ演出／進捗再計算フローを使い、スペルで盤面を変化させたら進捗バーと勝敗判定を即時同期する。
  - 設定値は難易度別に調整できるようパラメータ化し、今後のチューニングが容易な構成とする。
