...
- 概要
  ...
  - 画面
    - タイトル画面
    - レイアウト
      - タイトルは画面中央のやや左上へ配置し、`Tutorial` / `New Game` / `Continue` ボタンは縦並びで画面中央やや右下に配置する
        - ボタン順：最上段に `Tutorial`、中央に `New Game`、最下段に `Continue`
        - 各ボタン間隔は24pxを基準とし、フォーカスリングは共通スタイルで統一する
    - 「**Tutorial**」ボタンを表示する
      - **Tutorial** を押下した場合
        - チュートリアル用のデータを初期化する（ステップ進捗、練習盤面、入力履歴）
        - オープニングBGMを継続したままフェード扱いは行わずに `tutorial` モードへ遷移する
        - 遷移後、ヘッダー右上の状態表示に `Tutorial` を反映する
    - 「**New Game**」ボタンを表示する
      - **New Game** を押下した場合
        - 今までのプレイデータをクリアする。
          - ピクロスのプレイ履歴データを消去する。
...
- 概要
  - `Tutorial` ボタンから遷移する、ピクロスの基本操作と考え方を説明する専用モード。
  - 3ステップ構成（ゴール解説 → ヒント解説 → 練習パート）で進行し、全ステップ完了後はオープニングへ戻る。
  - 進行状況はセッション内でのみ保持し、ゲーム本編の進行データには影響しない。
- 表示
  - レイアウト
    - 左カラム：解説キャラクター用のポートレートカード（assets/img/character/tutorial/fiene_smile.png）を幅420px、高さ540pxで表示。
      - 背景色：青系グラデーション（#1e3a8a → #312e81、角丸16px）
      - 下部に説明タイトルとステップ進捗（例：Step 1 / 3）をバッジ表示する。
    - 右カラム：チュートリアル用の5x5ピクロス盤面を表示。
      - 盤面の周囲にヒント表示（行/列）を添え、必要に応じてハイライトオーバーレイを重ねられるようにする。
    - 画面下部：説明テキストウィンドウ（幅960px、高さ180px）と操作ボタン群を配置。
      - `Next` / `Back` / `Skip` ボタンを右寄せで並べ、`Skip` はモードを中断してオープニングに戻る。
      - 練習パートでは `Reset` ボタンを追加表示し、盤面状態を初期化できるようにする。
  - BGM / SE
    - オープニングBGM（assets/bgm/indie-game-pix-opening.wav）を継続再生し、ステップ切り替え時にボイスチャイムSE（assets/se/ui_confirm.wav）を小さめに再生する。
- 進行ステップ
  - Step1: ゴール説明
    - 盤面は半透明グリッド表示のみ（操作不可）。
    - テキスト：「ピクロスは、正しいマスをすべて Filled にできればクリアです。」
    - 自動デモ：1秒毎に指定セルをFilled化し、最後にクリアアニメーション（スコアリング風の光）を再生。
  - Step2: ヒント説明
    - 盤面は入力不可。行・列ヒントを順番にハイライトし、番号の意味をアニメーションで示す。
    - テキストでは `1 2 1` の例を具体的に表示し、「数字間は1マス以上空ける」「大きい数字から当てる」等を箇条書きで提示。
    - デモ後に「Next」で次ステップへ進める。
  - Step3: 練習パート
    - 盤面を操作可能にし、解答済み判定ロジックを有効にする。
    - 練習用パズル：5x5、解答パターンは以下の配列を使用（1=Filled,0=Empty）：
      - 行1: 0 1 1 0 0
      - 行2: 1 0 1 0 1
      - 行3: 1 1 1 0 0
      - 行4: 0 1 1 0 0
    ...
    - 列ヒント: `2`, `1 2`, `4`, `0`, `1 1`
    - 盤面のアフォーダンス：
      - Filled、×、Maybe の各入力を既存ピクロスモードと同じ方法で行える。
      - 正解セルを初めてFilledにした際、小さな蛍光エフェクトを表示。
      - 全セル解答一致で「よくできました！」ポップアップ（中央表示）→ `Return to Opening` ボタンを押すとモード終了。
    - クリアまたは `Skip` 選択時に、チュートリアル進捗を忘却してオープニングへ遷移する。
- 操作
  - `Next`：現在の説明が完了していれば次ステップへ進む。ステップ3では盤面クリア後に `Finish` 表示へ変化。
  - `Back`：前ステップへ戻り、盤面・ハイライト状態を該当ステップの初期状態へリセットする。
  - `Skip`：確認ダイアログ（Yes/No）表示後、Yes選択で即座にオープニングへ戻す。
- 内部状態
  - `tutorialStep`（0,1,2）で現在ステップを管理。
  - `tutorialBoard`：Step3で使用する盤面（プレイヤー入力＋答案）を保持。
  - `tutorialCompleted` フラグをセッション内で持ち、クリア済みの場合 `Tutorial` ボタン押下時に「内容を復習しますか？」確認ダイアログを挟む。
