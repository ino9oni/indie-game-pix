...
- 概要
  ...
  - スペル
    ...
    - スペル発動時は該当陣営のキャラクター画像に隣接するスペル表示ウィンドウ内に詠唱台詞を約4秒表示する。
      - HEROはHEROパネル（画面右配置）のHERO画像右隣にあるウィンドウへ表示する。
      - ENEMYはENEMYパネル（画面左配置）のENEMY画像左隣にあるウィンドウへ表示する。
      - HERO側は `${HERO_NAME}` が唱える戦闘台詞を表示し、敵側はスペルごとの専用台詞を表示する。
      - メッセージウィンドウにはテキストのみを表示し、キャラクター画像は含めない。
      - メッセージ領域のテキストは丸みのあるキュートな書体（例：`"M PLUS Rounded 1c", "Hiragino Maru Gothic Pro", "Rounded Mplus 1c", sans-serif`）を採用し、通常比200%の文字サイズで表示する。
        - フォント切り替えに合わせてメッセージ領域は最大280pxまで拡張し、自動折り返しで全台詞が収まるようにする。
      - スペルスロット（名称／説明／待機メッセージ）も同系統フォントへ統一し、従来比約140%のサイズで視認性を高める。
      - キャラクター枠／スペル枠／スコア枠は同一行で高さを統一し、盤面レイアウトのズレが発生しないようにする。
      - メッセージは再発動時に内容を更新し、表示時間経過または別陣営の発動時に個別に非表示へ戻る。
  ...
  - パズル生成
    - 各難易度ごとに「物語のモチーフを持つテンプレートバンク」を用意する。例：
      ...
      - easy：森の動物たちやエモート（小さな笑顔・驚き・ハートなど）を描く 10×10。
      - middle：紋章・古代文字・封印石など物語のキーアイテムを象る 5×5 の複合テンプレート。
      - hard：精霊の守り手や森の試練を表す 10×10（大樹・森の守護剣・双月など）。
      - ultra：終章を象徴する神器・星図・森羅の円環など 10×10 高密度テンプレート。
...
- 表示
  ...
  - HEROポートレート
    ...
    - レイヤー構成：
      - キャラクター画像自体は最前面に表示し、半透明のオーバーレイが人物に被らないようにする。
      - グローや装飾用のエフェクトは画像背面（z-indexを下げたレイヤー）で描画し、キャラクターの発色と輪郭を鮮明に見せる。
  - 勝利タイトル
