...
### タスク仕様（ENEMYのピクロスボードで×が表示されない不具合修正）

- 目的：ENEMY盤で「×」入力時にマークが表示されず枠線も消える不具合を解消し、HERO盤と同じ表示・挙動に統一する。
- 対象：ピクロスモード全難易度の ENEMY 盤表示・入力処理。
- 変更内容：
  - ENEMY 盤の `-1`（×）描画に HERO と共通のクラス／スタイルを適用し、マークとグリッド枠線が常に表示されるようにする。
  - ENEMY 盤の状態更新時（AI 誤答も含む）にマーク種別が欠落しないよう描画ロジックを見直す。
  - HERO 盤への影響が出ないことを確認し、他のセル状態（○/maybe/empty）の表示が崩れないことを確認する。
...
### タスク仕様（ENEMYの解答バーの進捗が解答があっていたとしても上がらない不具合）

- 目的：ENEMY が正しくマスを埋めても進捗バーが増えない／減り続ける不具合を解消し、敵の解答状況を正しく可視化する。
- 対象：ピクロスモードの ENEMY 側進捗バー（UI 表示および進捗計算ロジック）。
- 変更内容：
  - 進捗は「解答と一致する `1` セル数 ÷ 解答全体の塗りセル数」を毎手後に再計算し、`?` や誤答セルは進捗に含めない。
  - ロールバックや矛盾修正でセルが 0 に戻った場合は進捗を再計算して減算し、過小/過大カウントを防ぐ。
  - HERO の状態や入力とは独立して ENEMY 進捗を更新し、敵勝利判定もこの進捗に基づいて行う。
  - UI 進捗バーは 0〜100% をリニア表示し、更新は即時反映（キャッシュや遅延なし）。盤面サイズ 0／解答空の場合は 0% を表示。
  - 異常状態（解答なし・盤面サイズ不整合）が検出された場合は対象パズルを再ロードして復帰する。

### タスク仕様（ENEMY側がお題クリアにならない不具合修正）

- 目的：ENEMY が解答を完了してもクリア扱いにならない不具合を解消し、対戦の勝敗判定を正しく進行させる。
- 対象：ピクロスモードの ENEMY 勝利判定ロジック。
- 変更内容：
  - ENEMY 盤の進捗が解答と一致したとき（または設定した充足率以上に達したとき）、確実に ENEMY のクリアカウントを加算し、必要数に到達したら ENEMY 勝利とする。
  - HERO 盤の進捗や入力状態に依存せず、ENEMY 側のチェックは独立して行う。
  - 誤答やスペル効果で盤面が変化しても、再評価時にクリア判定が漏れないことを確認する。

### タスク仕様（エンドレスモードのヒント密度の下限保証）

- 目的：エンドレス一人プレイモードで 5×5 などの小型盤が「1/2 ばかり」で運頼みにならないよう、最低1行または1列に足がかりとなるヒントを必ず含める。
- 対象：エンドレスモードで事前・追加生成される全ピクロス（5×5／10×10を含む）。
- 変更内容：
  - 生成済みグリッドを採用する前に、行・列ヒントを解析し、以下の「アンカー行/列」条件を最低1本以上満たしているか検証する。満たさない場合は再生成し、一定回数失敗した場合は最も情報量の多い候補を採用する。
    - 5×5 基準例：`[4]` / `[5]` / `[3,1]` / `[1,3]` / `[1,1,1]` のいずれかを含む行または列。
    - サイズ一般化（10×10 等）：行/列に「最大ブロック長がサイズの 60%以上」または「ヒント合計がサイズの 60%以上」のものをアンカーとみなす。
  - 一意解チェックは従来どおり必須とし、アンカー判定後に再確認する。
  - ヒント分布が極端に大きすぎ／小さすぎに偏らないよう、既存のヒント正規化処理がある場合は併用する。
  - ストック補充（残 10% 未満時の追加生成）でも同じ基準を適用し、低品質パズル流入を防ぐ。

### タスク仕様（Middle モードの難易度緩和）

- 目的：Middle モードで敵が強すぎて勝てない状況を解消し、プレイヤーが攻略できる難易度に調整する。
- 変更内容：
  - HERO 側の盤面：5×5 盤で行/列の 20〜30% を「アンカー行列」（`[4]`, `[5]`, `[3,1]`, `[1,3]`, `[1,1,1]` など一意に近い手がかり）を必ず含むように出題をフィルタし、一意解を保証する。
...
### タスク仕様（ENEMYのAIがEASY以上で動作していない）

- 目的：EASY 以上の対戦で ENEMY の AI が動かなくなる／停滞する不具合を解消し、全難易度で安定して解答を進める。
- 対象：ピクロスモードの ENEMY AI ループ（推論・推定・勝利判定）。
- 変更内容：
  - スタック／ブラックリストが全候補を潰してしまう場合に、自動でブラックリストをクリアし再探索を開始する。
  - 連続スタック回数が閾値を超えたら強制的に成功率を 100%／誤答率 0% にして確定手を打つ「スタールブレイク」手に切り替え、停滞を解消する。
  - 推定手適用後の矛盾検出時には、直前スナップショットにロールバックして別セルを選択する。ロールバックしきれない場合はリストを再構成する。
  - EASY 以上の全難易度で上記ロジックを有効化し、HERO の状態に依存せず独立タイマーで解答を継続する。
