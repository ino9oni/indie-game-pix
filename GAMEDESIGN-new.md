...
### タスク仕様（ピクロスモードで、HEROとENEMYとでレイアウトの高さがあっていない）

- 目的：ピクロス画面で HERO / ENEMY パネルの高さがずれる問題を解消し、見た目と情報量を揃える。
- 対象：ピクロスモードのレイアウト全体（キャラ枠、スペル台詞枠、スコア枠、お題数枠、ボード枠、進捗バー枠、ヒント数字表示）。
- 変更内容：
  - HERO / ENEMY パネルの縦グリッドを統一し、キャラクター表示・スペル台詞・スコア＋お題数・ボード・進捗バーが同じ行高になるよう CSS を再定義する。
  - ボード枠は 5×5 / 10×10 いずれでも同じ縦幅に収まるよう、コンテナ高さを共通化し内部セルをスケールさせる。片側だけ縮む／伸びる状態を防ぐ。
  - ヒント数字（行列）の最小高さと行間を固定し、桁数が増減しても上下位置がずれない余白を確保する。
  - ウィンドウ幅でレイアウトが縦並びに切り替わる場合も、高さ比率を維持し、HERO/ENEMY いずれかが突出しないことを確認する。
  - 配色や余白の既存デザインは維持しつつ、左右のパネルで視覚的な高さ差異が生じないようにする。
