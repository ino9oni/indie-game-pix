...
- `route-svg` の高さ指定を `height: clamp(1200px, 70vh, 1200px)` に変更し、ビューポート横幅に合わせて縦方向が過度に伸長しないよう制約を設ける。
- 変更後もリサイズ／デバッグジャンプ時に座標再配置が破綻しないこと、ゲームパッド操作・アクセシビリティ設定が従来通り機能することを確認する。
