...
### タスク仕様（ENEMY側が適当に回答をしていても正解している不具合を修正）

- 目的：ENEMYがヒント無視のランダム入力で勝利する不具合を解消し、常にピクロスの解法ロジックに従って進行させる。
- 対象：ピクロス対戦時のENEMY回答ロジックおよび勝利判定。
- 変更内容：
  1. 回答生成  
     - ENEMYは解答を事前に知らない前提とし、行列ヒントから導出した候補セルのみを塗る。また、塗布時には列・行ヒントを超過しないよう制約チェックを行う。  
     - 探索が行き詰まった場合のみ限定的な推測を許可し、推測回数や頻度を難易度に応じて制限する（例：easyは推測多め・hard/ultraは低頻度）。
  2. 勝利判定  
     - ENEMYの勝利は「塗りマス完全一致（解探索）かつクリア数＝目標数」のときのみ成立するようにし、ランダム一致・部分一致では勝利にならないようガードを入れる。  
     - 判定時にグリッドと解集合（ヒント整合チェックを含む）の整合性を検証し、不整合ならENEMY敗北扱いとする。
  3. レート調整  
     - ENEMYの思考時間・ミス率は難易度ごとに維持しつつ、ヒント整合チェックに要する時間を加味して1手あたりの処理を調整する（過度な高速化を避ける）。

### タスク仕様（スペル発動時のエフェクトが行われなくなっている不具合）

- 目的：スペル発動時にエフェクトが表示されない不具合を解消し、シネマティック→攻撃演出→結果表示が必ず実行されるようにする。
- 対象：HERO／ENEMY のスペル発動全般（演出レイヤーとスペル適用ロジック）。
- 変更内容：
  1. 演出トリガーの再有効化  
     - スペル発動時にシネマティックオーバーレイを必ず表示し、タイマー停止・入力ロックを行う。  
     - スキップ設定が有効な場合は演出を短縮またはスキップするが、攻撃結果（ボードヒット）は必ず適用する。
  2. 攻撃エフェクトの確実な適用  
     - スペル効果適用時に、攻撃対象セル周辺のハイライト・フレア・揺れなどのエフェクトを必ず起動し、対象ボード上に重ねて表示する。  
     - キャスター/ターゲットに応じて貼り付け先レイヤーを切り替え（HERO→enemy board、ENEMY→hero board）、attach漏れを防止する。
  3. 復帰シーケンス  
     - 攻撃終端後に短い静止時間（400〜600ms）を確保し、被弾状態を見せてから入力・タイマーを再開する。  
     - エフェクト終了までに例外が発生しても、ボード状態とタイマーが不整合にならないようフェイルセーフを入れる。
