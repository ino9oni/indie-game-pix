...
- 目的：マップ画面で進行中のストーリーを右側に常時表示し、プレイヤーが現在の進行状況を把握できるようにする。
- 対象：マップモードのUIレイアウト（右側パネル追加／表示コンポーネント）。
- 変更内容：
  - マップ画面右側に縦長のストーリーパネルを追加し、これまでの重要イベント・会話の要約を時系列で表示する。
  - ストーリーパネルはスクロール可能とし、最新エントリを上部固定表示、既読分を下に時系列で積み上げる。
  - 各エントリは「章タイトル」「短い要約（1〜2行）」「発生ノード名」を含め、長文は折りたたみボタンで展開できる。
  - マップ表示領域との横幅配分は可変（例: 左マップ 65〜70%、右パネル 30〜35%）で、ウィンドウ幅に応じレスポンシブに再配置（狭幅時は下部にドッキング）。
  - 進行に応じたハイライト表示（最新行を強調）と、UIトグル（表示/非表示）をヘッダーに追加する。
  - データは既存の進行状態から生成し、ストーリーパネル追加によるマップ操作への遅延が生じないよう非同期更新/メモ化を行う。
