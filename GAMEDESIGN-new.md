...
- 内部データ表現
  ...
  - ルートとノードの表現
    ...
    - ノード配置と進行方向
      - 進行するほど右上へ伸びていくレイアウトとし、スタート地点を左下寄り、ゴール地点を右上寄りに配置して上方向への進行感を演出する
      - SVG座標系（viewBox: 0 0 800 400）での基準座標
        - start: (120, 320)
        - elf-practice: (240, 260)
        - elf-easy: (360, 210)
        - elf-middle: (500, 160)
        - elf-hard: (640, 120)
        - elf-ultra: (760, 80)
...
- 画面
  ...
  - ルートは直角の折れ線で表示する
    - 各エッジはノード中心から水平方向へ伸びた後に垂直方向へ折れ、L字またはコの字を構成する
    - 曲線表現は使用しない
  - 背景画像
    - assets/img/\* からランダムな画像を背景に表示
      - 透明度：60%
...
- 操作
  ...
  - スコア加算演出
    ...
    - ステージクリア時の従来ファンファーレ（別効果音）は再生しない
  - クリア後の演出
