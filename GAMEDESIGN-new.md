...
### タスク仕様（ゲーム開始時のデバッグモードのステータス変更）

- 初回起動やデバッグ設定が未保存の状態では、デバッグメニューの各トグルを既定で Off とする。
  - `即時クリア`（お題サブミット時に即座にクリア扱いにする挙動）：既定 Off。
  - `マップ全体表示`（移動先以外のノードも常時表示するデバッグ表示）：既定 Off。
- トグル操作時には従来どおり `localStorage` へ保存し、保存済みの値が存在する場合はその値を起動時に復元する。
- ニューゲームやセーブデータ初期化でデバッグ設定をリセットした際も、リセット後の初期状態は上記既定値 Off を適用する。
