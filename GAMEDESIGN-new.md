...
- 概要
  ...
  - スペル
    ...
    - スペル発動時は該当陣営のキャラクター画像右側に吹き出しを表示し、詠唱台詞を約4秒間表示する。
      ...
      - 吹き出し内テキストは丸みのあるキュートな書体（例：`"M PLUS Rounded 1c", "Hiragino Maru Gothic Pro", "Rounded Mplus 1c", sans-serif`）を採用し、通常比200%の文字サイズで表示する。
        - フォント切り替えに合わせて吹き出し幅は最大280pxまで拡張し、自動折り返しで全台詞が収まるようにする。
