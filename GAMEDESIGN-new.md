...
- Symbol Codex ボードは 30 スロットを保持し、未解放は `??` を表示、解放済みは対応するシルエットと記号情報を点灯させる。

### タスク仕様（勝利画面レイアウトの改善）

- 勝利画面のメインレイアウトを左列・右列の2カラムに再構成し、左列の上段にHEROポートレート＋勝利メッセージ、左列の下段に新規アンロック情報を配置する。右列はSymbol Codexボードを全面に展開する。
- Symbol Codexボードは画面幅の約80%（最大1040px程度）を確保し、8列×3行のグリッドを基本構成とする。4行目以降に項目が溢れる場合は自動で列を追加せずスクロールで対応する。
- Codexスロットは5×5のピクロスシルエットを保持しつつ、枠内の余白を0.2px相当（視覚的にほぼ無し）まで縮小し、左右パディングは1pxに抑える。
- スロットのテキスト表示は記号名称（`meaningText`）のみとし、難易度ラベルや解放日付は非表示にする。名称は1行で収まらない場合は折り返し。
- 難易度ごとに背景色を割り当てる：practice＝淡い半透明グリーン、easy＝淡い半透明ブルー、middle＝淡い半透明パープル、hard＝淡い半透明レッド、ultra＝淡い半透明ブラック。
- HERO列・アンロック列・ボード列が互いに干渉しないよう、レイアウト全体の高さを最大90vhに収めつつコンテンツを中央に配置する。
...
- 表示
  ...
  - レイアウト
    - デスクトップ幅では左右2カラム（左列：HERO／アンロック、右列：Symbol Codex）に並べ、`gap: 32px` 程度を確保する。
    - モバイル幅（≤768px）では縦積みに切り替え、HEROセクション → アンロック一覧 → コレクションボード → ボタンの順で中央揃えにする。
  - HEROセクション（左カラム）
  ...
  - コレクションボード（右カラム）
    - 木目調のボードパネルを幅80%程度に拡張し、内部は8列×3行（計24セル）のグリッドを基準とする。セル数が24を超える場合は縦方向にスクロールで対応し、列数は増やさない。
    - 各スロットは丸みを帯びたソケットで表現し、未解放時は透過度60%、解放時は難易度別の半透明カラー（practice：薄緑、easy：薄青、middle：薄紫、hard：薄赤、ultra：薄黒）で発光させる。
    - グリッドのセル間隔は左右1px、上下はシルエットが潰れない範囲で0.2px相当に抑え、ボードパネル内の余白も最小限にする。
    - 今回新たに埋まったスロットは 0.6s のポップアップアニメーション（スケール1.2→1.0、アルファ1.2→1.0）を再生し、既存のスロットは静的状態で表示する。
  - 記号ピース表示
    - ピース中央にはピクロスシルエットを表示し、テキストは記号名称（`meaningText`）のみを表示する。難易度タグや解放日付は表示しない。
