...
### タスク仕様（エンドレス一人プレイ用のモードを追加）

- 目的：ストーリーモード以外に、CPU対戦なしでピクロスを好きなだけ解けるエンドレスモードを提供する。
- 入口：オープニング画面に「エンドレスモード」を追加し、既存の New Game は「ストーリーモード」に名称変更。各ボタンには吹き出し説明を付与。
  - 吹き出しはボタンの右上に表示し、マウスホバーまたはキーボードフォーカス時のみ出現、ボタンを覆わない。フォーカス/ホバーが外れたら非表示にする。
  - ストーリーモード吹き出し：「ストーリーに沿って elfpix を進めるモードです」
  - エンドレスモード吹き出し：「エンドレスな elfpix にチャレンジするモードです」
- エンドレスモード仕様：
  - 盤面サイズは開始時に 5×5 / 10×10 から選択できる。
  - マップ／会話は非表示。敵ボードも表示せず、HEROボードのみ中央表示。
  - お題生成は開始前に 20 問程度まとめて行い、残り 10% になったら追加生成する。生成に時間がかかる場合はローディング表示可。
  - 生成するお題は一意解で、ヒントは過度に大きい/小さい数字に偏らないようバランスをとる（既存の生成器を流用し、ヒント正規化を適用）。
  - Quit押下で終了確認を行い、OKならエピローグ画面へ戻す。
