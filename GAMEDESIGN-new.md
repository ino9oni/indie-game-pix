...
### タスク仕様（チュートリアルにコンボ・スペル・ジャマーの説明を入れる）

- 既存チュートリアルの基本解説（ルール説明→実演）が終わったタイミングで、「このままオープニングに戻る」「コンボ・スペルの説明に移る」の２択ボタンを表示する。
  - 選択肢は現在のチュートリアルウィンドウ内に縦並びで配置し、デフォルトフォーカスは「コンボ・スペルの説明に移る」。
  - どちらを選んでも BGM / 既存演出は継続し、追加ロードを挟まない。
- 「コンボ・スペルの説明に移る」を選ぶと、以下のスライドを `Next` / `Back` で巡回できる 6 枚構成の追加レッスンへ遷移する。各スライドは従来のチュートリアル枠と同じスタイルで、見出し＋本文＋ミニ図解（ピクセル化したアイコン）をセットで表示する。
  1. **コンボとは** — 正解／不正解を連続で取った回数を表示し、1 手でも外すとリセットされる旨を解説。盤面サンプルを添えて「Combo 3 ➝ ミスで 0」アニメーションを表示する。
  2. **コンボになる条件** — 正解セルの塗り潰しと、正しい × マークでもカウントが進むことを箇条書きで提示し、誤タップや正解セルの解除では途切れることを明記する。
  3. **スペルとは** — 各キャラクター固有の必殺技であり、スペル発動＝ジャマー起動であることを説明する。UI 上はスペルボタンが点灯した状態のモックを並べ、キャラ立ち絵が吹き出しでセリフを出す演出を図示する。
  4. **ジャマーとは** — 相手盤面へのダメージ（塗り戻し・時間停止など）を与えて攻略を妨害する効果で、コンボ段階（Lv1〜Lv5）に応じ威力が上がることを示す。代表的な効果例を 2〜3 個提示する。
  5. **スペル発動条件と使い方** — コンボ数が閾値に達するとスペルボタンがアクティブになる／クリックで即発動しコンボはリセットされる、という操作手順をステップ図で解説する。
  6. **戦略のヒント** — コンボを溜めて強いジャマーを狙う vs 早期に撃って安全圏を作る等、リスク・リターンを比較し、最後に「自分のプレースタイルを探そう」と締める。
- 追加レッスン中は画面右上に進捗インジケーター（`1/6` 形式）と `Skip` ボタンを配置し、`Skip` 押下時は即座にチュートリアル完了ダイアログへ遷移する。`Back` は 1 枚目では無効化（グレーアウト）する。
- レッスン完了後は確認メッセージ「コンボ・スペル解説を終了します。オープニングへ戻ります。」を表示し、確定でオープニング画面へ遷移させる。戻る際にチュートリアル進捗フラグ `tutorialComboTipsSeen` を `true` 保存し、次回チュートリアル開始時に「コンボ解説のみを見る」ショートカットを提供する。
- 文章・UI テキストは上記箇条書きに沿って実装し、翻訳や将来のボイス追加に備えて各スライドのキー（例：`tutorial.combo.definition.title`）を `src/strings/tutorial.json` に追加する。
