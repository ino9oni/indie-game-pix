...
- ゲームパッド検出
  - Web Gamepad API を用いて、接続された最初の標準コントローラ（Xbox / DualShock レイアウト相当）を自動検出する
  - 接続・切断イベントを監視し、未接続時は既存のマウス／キーボード操作へフォールバックする
- タイトル・メニュー操作
  - 左スティックまたは十字キー左右でメニュー項目を巡回し、現在選択中のボタンを視覚的に強調表示する
  - `A / Cross` ボタンで決定（New Game / Continue の押下）、`B / Circle` でキャンセル・前画面へ戻る（可能な画面のみ）
- 会話／テキスト進行
  - `A / Cross` で次のテキストを表示、`B / Circle` でスキップ、`Start` でスキップ確認ダイアログを開く
- ピクロス操作
  - 十字キーまたは左スティックで、盤面上に用意したゲームパッド用カーソルを上下左右へ移動する
  - `A / Cross` ボタンでカーソル位置のマスを Fill、`B / Circle` で Cross、`Y / Triangle` で Maybe マークをトグルする
  - `X / Square` ボタンでマスの入力をクリア、`Start` で Pause / Resume を切り替える、`Select` で Reset を呼び出す
  - コントローラカーソル位置は 1 マス分のハイライト枠として描画し、マウス操作と同等にスコア演出やスペル判定が動作する
- フォーカス管理
  - ゲームパッド利用時は、最終操作デバイスに応じてフォーカスリング／カーソル表示を切り替え、マウスに戻った際は自動で従来の挙動へ戻る
