...
- 概要
  ...
  - パズル生成
    ...
    - テンプレートからパズルを生成する際は、回転・反転・平行移動に加え、複数テンプレートの合成（例：主図形＋アクセントモチーフ）や部分削除／装飾追加を行い、視覚的に明確な差異が出るようにする。合成時も意味が通るよう事前に定義した「組み合わせルール」を遵守する（例：ハート＋星の飾り＝祝福の紋章）。
    - 生成時は左右／上下の完全対称となるレイアウトを原則禁止とし、もし生成された場合は別の変換を再試行する。直近に出題したレイアウトはハッシュで管理し、一定数（例：40件）までは再利用しない。
    - 生成した盤面は `computeClues` 相当のヒントを算出し、バックトラッキング型ソルバで唯一解かを検証する。複数解が見つかった場合は別のテンプレート／変換で再生成する。
    - HERO と ENEMY は同じ生成済みパズル集合を共有するが、提示順は必ず別々にシャッフルし、毎戦ごとに order が異なる。必要に応じて HERO/ENEMY それぞれのシードを派生させる。
    - 生成ロジックは乱数シードを受け取り（指定が無い場合は毎回ランダムシード）、デバッグ用途で seed を UI またはデバッグログで確認できるフックを用意する。
    - 返却値は各パズル毎に `grid`, `rowClues`, `colClues` に加えて `glyphMeta`（上記4プロパティ＋`description`、`unlockedAt` 等の拡張フィールドを許容）を含め、UI 層が勝利演出に即座に利用できる形にする。
...
- 表示
  ...
  - HEROセクション（左カラム）
    ...
    - 勝利タイトル：`<HERO_NAME> WIN` をポートレート下に左揃えで表示し、サブテキストで今回獲得したシンボル名（例：「新たな記号：導きの矢印」）を1行表示する。
  - コレクションボード（右カラム）
  ...
  - 記号ピース表示
    - `glyphLabel` をスロット中央に大きく描画し、下部には小さめの英数字で難易度タグ（例：`EASY-03`）と解放日付を表示する。
