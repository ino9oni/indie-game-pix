  ...
  - 主人公の画像
    ...
    - 敵の画像
      ...
      - フレーム：幅570px × 高さ731px（ベース値）で表示し、必要に応じてレスポンシブに縮小する
        - 画像は表示枠いっぱいに拡大（object-fit: cover）
        - 表示枠を超えた部分はマスクしてトリミングする
        - 表示枠の背景色：深い赤紫のグラデーション（例：手前 #631b4a → 奥 #350a2b）で、敵側と主人公側の色調差を明確にする
...
- 概要
  ...
  - レイアウト
    ...
    - 盤面のグリッド表現
      ...
      - `×`（Maybe）表示はセル中央に等幅で配置し、縮小・拡大時も上下左右の余白が均等になるよう基準サイズを定める（ライン高さ=セルサイズ、letter-spacing=0）。
        - 文字アウトラインがセル境界にめり込まないよう、描画時に `transform` や `line-height` 調整で縦方向のずれを防止する。
