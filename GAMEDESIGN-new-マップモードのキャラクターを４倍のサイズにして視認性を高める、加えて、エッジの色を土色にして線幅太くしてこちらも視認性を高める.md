...
### タスク仕様（マップキャラクター拡大と土色エッジ強調）

- 目的：マップ上のキャラクター/ルートの視認性を大幅に高める。
- 対象：ルートマップのキャラクター画像、エッジ（ルート線）。
- 変更内容：
  - マップモードのキャラクター表示 PNG を縦横 4 倍で描画する（拠点/敵/分岐のキャラ表示も同様）。
  - 画像拡大に伴い、アイコンの重なりが起きないよう必要に応じて配置と余白を再調整する。
  - エッジ色は土色（例：#8b5a2b 付近）へ変更し、線幅は 10px 程度まで太くする。
  - 上記変更はマップモードのみで適用し、他画面には影響させない。
